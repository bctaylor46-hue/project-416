...
const rpeClr = v => v >= 10 ? G.red : v >= 8 ? "#f97316" : v >= 6 ? G.yellow : G.teal;
...
const FC    = {"Upper Body":G.teal,"Lower Body":G.teal,"Full Body":G.teal};
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
      {f.opts.map(o => { const sel = mev[f.key]===o.v; return (
        <button key={o.l} onClick={()=>tog(f.key,o.v)}
          style={{padding:"5px 11px",borderRadius:4,border:sel?`1px solid ${G.teal}`:`1px solid ${G.border}`,
            background:sel?`${G.purple}20`:"#0a0a0a",color:sel?G.teal:G.muted,fontSize:11,cursor:"pointer",fontWeight:sel?700:400}}>
          {o.l} <span style={{fontSize:9,color:sel?G.teal:"#333"}}>({o.v>0?"+":""}{o.v})</span>
        </button>
      );})}
    </div>
  </div>
  ...
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <span style={{fontFamily:F.display,fontSize:14,color:val?G.teal:G.muted,letterSpacing:1}}>
          {val ? `${score>0?"+":""}${score}` : "—"}
        </span>
      </div>
...
const SH = ({children,color}) => (
  <div style={{fontFamily:F.display,fontSize:13,color:color||G.teal,letterSpacing:4,marginBottom:10,paddingBottom:8,borderBottom:`1px solid ${color||G.teal}25`,display:"flex",alignItems:"center",gap:10}}>
    {children}
  </div>
);
          ...
          <div style={{display:"flex",gap:6,marginBottom:22}}>
            {OB_STEPS.map((s,i)=>(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                <div style={{width:"100%",height:2,borderRadius:99,background:i<step?G.teal:i===step?G.purple:"#1a1a1a"}}/>
                <span style={{fontSize:8,color:i===step?G.teal:i<step?G.muted:"#2a2a2a",fontWeight:700,letterSpacing:1}}>{s.toUpperCase()}</span>
              </div>
            ))}
          </div>
        )}
          ...
          {step===2 && (
            <div>
              ...
              <div style={{background:"#0f0f0f",border:`1px solid ${G.border}`,borderRadius:8,padding:"12px 14px",marginBottom:16}}>
                ...
                <div style={{fontSize:28,fontWeight:900,color:mevScore>0?G.teal:mevScore<0?G.red:G.teal}}>
                  {mevScore>0?"+":""}{mevScore}
                </div>
    ...
    <div>
      <div style={{background:G.card,borderRadius:10,padding:"12px 16px",border:`1px solid ${G.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          ...
          <div style={{fontSize:26,fontWeight:900,color:volScore>0?G.teal:volScore<0?G.red:G.muted}}>{volScore>0?"+":""}{volScore}</div>
        </div>
        <button onClick={()=>setShowRef(s=>!s)}
          style={{background:showRef?`${G.teal}12`:"#0a0a0a",border:`1px solid ${showRef?"#2a2a2a":G.border}`,
            borderRadius:6,color:showRef?G.teal:G.muted,padding:"8px 14px",fontSize:10,fontWeight:700,letterSpacing:2,cursor:"pointer"}}>
            ...
            <thead>
              <tr style={{background:"#111111"}}>
                {["Body Part","MV","MEV","MAV","MRV","Freq","Rep Range"].map(h=>(
                  <th key={h} style={{padding:"6px 8px",fontSize:9,color:G.teal,fontWeight:800,letterSpacing:1,textAlign:"left"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {BODY_REF.map(([name,...vals],i)=>(
                <tr key={name} style={{background:i%2?"#111111":"transparent"}}>
                  <td style={{padding:"6px 8px",fontSize:11,fontWeight:700,color:G.teal}}>{name}</td>
      ...
      <div style={{display:"flex",gap:7,marginBottom:14}}>
        {PROGRAM.map(w=>{const act=activeWeek===w.week;return(
          <button key={w.week} onClick={()=>setActiveWeek(w.week)}
            style={{flex:1,padding:"8px 4px",borderRadius:7,border:act?`1px solid ${G.teal}`:`1px solid ${G.border}`,
              background:act?`${G.teal}15`:"#0a0a0a",color:act?G.teal:G.muted,fontFamily:F.display,fontSize:15,cursor:"pointer",letterSpacing:3}}>
            WK {w.week}
          </button>
        );})}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {PATS.map(pat=>{
          ...
          const SI={none:{l:"No Volume",c:"#333"},below:{l:"Below MEV",c:G.red},good:{l:"In Range",c:G.teal},near:{l:"Near MRV",c:G.purple},over:{l:"Over MRV",c:G.red}}[status];
          const bcLine={none:"#333",below:G.red,good:G.teal,near:G.purple,over:G.red}[status];
          const patEx=[];
          weekData.sessions.forEach(s=>s.exercises.forEach(ex=>{
          ...
          return(
            <div key={pat.id} style={{background:G.card,borderRadius:10,border:`1px solid ${G.border}`,padding:14}}>
              ...
              <div style={{background:"#1a1a1a",borderRadius:99,height:6,marginBottom:7,position:"relative",overflow:"visible"}}>
                ...
                {adjMev>0&&<div style={{position:"absolute",top:-3,left:`${Math.min(99,(adjMev/adjMrv)*100)}%`,width:1,height:12,background:G.teal,opacity:0.6}}/>}
              </div>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <span style={{fontSize:9,color:G.teal}}>MEV: {adjMev}</span>
...
const ProtoTag = ({proto,exName,onTimer}) => (
  <div style={{display:"flex",gap:5,alignItems:"center"}}>
    <span style={{fontFamily:F.display,fontSize:12,background:`${G.teal}20`,color:G.teal,padding:"1px 8px",letterSpacing:2}}>{proto}</span>
    <button onClick={()=>onTimer({proto,exName})}
      style={{background:"#0a0a0a",border:`1px solid ${G.teal}40`,borderRadius:3,color:G.teal,padding:"2px 7px",fontSize:9,cursor:"pointer",letterSpacing:1,fontWeight:700}}>
      ▶ TIMER
    </button>
  </div>
);
const ProtoTagAcc = ({proto,exName,onTimer}) => (
  <div style={{display:"flex",gap:5,alignItems:"center",flexShrink:0}}>
    {proto&&<span style={{fontSize:10,color:G.teal,background:`${G.teal}20`,borderRadius:3,padding:"2px 7px",fontWeight:700,letterSpacing:1}}>{proto}</span>}
...
function MainBlock({ex,rms,unit,sid,onLog,logs,onTimer,readinessAdj=0}) {
  ...
  return (
    <div style={{border:`1px solid ${G.border}`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:G.card2,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,borderTop:`1px solid ${G.teal}`}}>
        <span style={{fontFamily:F.display,fontSize:20,color:G.teal,letterSpacing:2,lineHeight:1}}>{ex.name}</span>
      ...
      <div style={{display:"flex",alignItems:"center",background:"#111111",padding:"6px 14px"}}>
        ...
        <div style={{width:72,textAlign:"right",fontSize:9,color:G.teal,fontWeight:700,letterSpacing:1,flexShrink:0}}>WEIGHT</div>
      </div>
      {ROWS.map(row=>(
        <div key={row.ro} style={{display:"flex",alignItems:"center",borderTop:`1px solid ${G.border}`,padding:"9px 14px",background:row.isTop?`${G.teal}12`:"transparent"}}>
          <div style={{width:24,flexShrink:0}}><span style={{fontSize:10,color:G.teal,fontWeight:700}}>{row.ro}</span></div>
          <div style={{flex:"1 1 auto",fontSize:12,color:row.isTop?G.teal:G.muted,fontWeight:row.isTop?700:400}}>{row.lb}</div>
          <div style={{width:34,textAlign:"center",fontSize:15,fontWeight:900,color:row.isTop?G.teal:G.text,flexShrink:0}}>{row.reps}</div>
          <div style={{width:96,textAlign:"center",fontSize:10,color:"#444",flexShrink:0}}>{row.tag}</div>
          <div style={{width:72,textAlign:"right",flexShrink:0,fontSize:row.isTop?17:13,fontWeight:row.isTop?900:600,color:row.isTop?G.teal:G.text}}>
      ...
      <div style={{padding:"9px 14px",borderTop:`1px solid ${G.border}`,background:"#111111"}}>
        {top
          ?<button onClick={()=>{onLog(sid,ex.name,{topWeight:top});setLogged(true);}} disabled={logged}
            style={{padding:"6px 14px",background:logged?`${G.teal}15`:"#0f0f0f",border:logged?`1px solid ${G.teal}40`:`1px solid ${G.border}`,
              borderRadius:4,color:logged?G.teal:G.muted,fontSize:10,fontWeight:700,letterSpacing:2,cursor:logged?"default":"pointer"}}>
...
function RpeBlock({ex,sid,accLogs,setAccLog,unit,onTimer,nested=false}) {
  ...
  return (
    <div style={{border:nested?"none":`1px solid ${G.teal}40`,borderRadius:nested?0:8,overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",background:"#111111",padding:"5px 14px"}}>
        ...
        <div style={{width:80,textAlign:"right",fontSize:9,color:G.teal,fontWeight:700,letterSpacing:1,flexShrink:0}}>WEIGHT</div>
      </div>
      {ex.sets.map((set,i)=>{
        const wt=exLogs[i]||"";
        return (
          <div key={i} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:`1px solid ${G.teal}20`}}>
            <div style={{width:24,flexShrink:0}}><span style={{fontSize:10,color:G.teal,fontWeight:700}}>{i+1}</span></div>
            ...
            <div style={{width:82,textAlign:"center",flexShrink:0}}>
              {set.rpe!=null
                ...
                :<span style={{fontSize:10,color:G.teal,fontStyle:"italic",whiteSpace:"nowrap"}}>{set.note||"= prev"}</span>
              }
            </div>
            <div style={{width:80,textAlign:"right",flexShrink:0}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:3}}>
                <input type="number" value={wt} onChange={e=>setAccLog(sid,ex.name,i,e.target.value)}
                  style={{width:46,background:"#0a0a0a",border:`1px solid ${G.teal}30`,borderRadius:4,
...
function SupersetBlock({ex,sid,accLogs,setAccLog,unit,onTimer}) {
  return (
    <div style={{border:`1px solid #2a2a2a40`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:"#0f0f0f",padding:"7px 14px",borderBottom:"1px solid ${G.purple}25",display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:9,color:G.teal,fontWeight:900,letterSpacing:2,background:`${G.teal}18`,borderRadius:3,padding:"2px 8px"}}>SUPERSET</span>
        <span style={{fontSize:10,color:G.teal}}>{ex.exA.name} + {ex.exB.name}</span>
      </div>
      <RpeBlock ex={ex.exA} sid={sid} accLogs={accLogs} setAccLog={setAccLog} unit={unit} onTimer={onTimer} nested/>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"4px",background:"#111111",borderTop:"1px solid ${G.purple}25",borderBottom:"1px solid ${G.purple}25"}}>
...
function ReadinessGate({onComplete}) {
  ...
  return (
    <div style={{padding:"20px 16px"}}>
      ...
      <div style={{fontFamily:F.display,fontSize:28,color:G.teal,letterSpacing:3,marginBottom:6,lineHeight:1}}>HOW ARE YOU SHOWING UP?</div>
      ...
      {READINESS_Qs.map(q=>{
        ...
        const c = q.inverted
          ? (val<=4?G.teal:val<=7?G.teal:G.muted)
          : (val>=7?G.teal:val>=4?G.purple:G.muted);
        return (
          <div key={q.id} style={{marginBottom:20}}>
            ...
            <input type="range" min={1} max={10} value={val}
              ...
              style={{width:"100%",WebkitAppearance:"none",appearance:"none",height:3,borderRadius:2,
                background:`linear-gradient(90deg, ${G.teal} ${pct}%, #222 ${pct}%)`,outline:"none",cursor:"pointer"}}/>
          </div>
        );
      })}
      ...
      <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"14px 16px",marginBottom:20,marginTop:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
          ...
          <span style={{fontFamily:F.display,fontSize:20,color:G.teal,letterSpacing:3}}>{prof.label}</span>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,padding:"0 4px"}}>
        ...
        <span style={{fontFamily:F.display,fontSize:28,color:G.teal,letterSpacing:1}}>{score}/10</span>
        <div style={{flex:1,height:1,background:G.border}}/>
      </div>
...
const qColor = v => v>=8?G.teal:v>=6?G.purple:v>=4?G.muted:"#444";
...
function WinsScreen({readiness,sessionId,completed,qualityRatings,onClose}) {
  ...
  return (
    <div style={{padding:"28px 16px",textAlign:"center"}}>
      ...
      <div style={{fontFamily:F.display,fontSize:48,color:G.teal,letterSpacing:3,lineHeight:1,marginBottom:6}}>YOU SHOWED UP.</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
        {[
          ...
          <div key={label} style={{background:G.card2,border:`1px solid ${G.border}`,borderTop:`1px solid ${G.teal}`,padding:"16px 10px"}}>
            <div style={{fontSize:20,marginBottom:6}}>{icon}</div>
            <div style={{fontFamily:F.display,fontSize:22,color:G.teal,letterSpacing:2,lineHeight:1}}>{val}</div>
      ...
      {nextSess&&(
        <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"14px 16px",marginBottom:20,textAlign:"left"}}>
          ...
          <div style={{fontFamily:F.display,fontSize:18,color:G.teal,letterSpacing:2}}>S{nextSess.id} — {nextSess.day.toUpperCase()}</div>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div id="session-share-card" style={{
          background:G.black,border:`1px solid ${G.teal}40`,padding:"24px 20px",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${G.border}`}}>
            ...
            <div style={{textAlign:"right"}}>
              <div style={{fontFamily:F.display,fontSize:22,color:G.teal,letterSpacing:2,lineHeight:1}}>S{sessionId}</div>
          ...
          <div style={{display:"flex",gap:16,marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${G.border}`}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontFamily:F.display,fontSize:28,color:G.teal,letterSpacing:2,lineHeight:1}}>{readiness.score}<span style={{fontSize:14,color:G.muted}}>/10</span></div>
              <div style={{fontFamily:F.display,fontSize:11,color:G.teal,letterSpacing:2,marginTop:3}}>{readiness.profile.icon} {readiness.profile.label}</div>
            </div>
            <div style={{flex:2}}>
              <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:6}}>PROGRAM</div>
              <div style={{fontFamily:F.display,fontSize:22,color:G.teal,letterSpacing:2,lineHeight:1,marginBottom:6}}>{completed.size}<span style={{fontSize:12,color:G.muted}}>/16 SESSIONS</span></div>
            ...
            if (ex.type==="main") {
                ...
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div>
                      ...
                      <div style={{fontFamily:F.display,fontSize:18,color:G.teal,letterSpacing:2,lineHeight:1}}>{ex.name.toUpperCase()}</div>
                    </div>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {["i","ii","iii","iv","v"].map((ro,ri)=>(
                      <div key={ro} style={{background:"#0a0a0a",border:`1px solid ${ri===2?G.teal:G.border}`,padding:"4px 8px",textAlign:"center",minWidth:44}}>
                        <div style={{fontFamily:F.display,fontSize:8,color:ri===2?G.teal:G.muted,letterSpacing:1}}>{ro}</div>
                        <div style={{fontFamily:F.display,fontSize:13,color:ri===2?G.teal:G.text,letterSpacing:1}}>{ex.pat?.reps[ri]}</div>
                      </div>
                    ))}
                    <div style={{background:`${G.teal}15`,border:`1px solid ${G.teal}40`,padding:"4px 8px",textAlign:"center",alignSelf:"center"}}>
                      <div style={{fontFamily:F.display,fontSize:8,color:G.teal,letterSpacing:1}}>TARGET</div>
                      <div style={{fontFamily:F.display,fontSize:13,color:G.teal,letterSpacing:1}}>{ex.pat?.tl}{readiness.adj?` ${readiness.adj}%ADJ`:""}</div>
                    </div>
                  </div>
                </div>
              );
                ...
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  ...
                  {[ex.exA, ex.exB].map((e,j)=>(
                    <div key={j} style={{marginBottom:6}}>
                      <div style={{fontFamily:F.display,fontSize:15,color:G.teal,letterSpacing:2,marginBottom:3}}>{e.name.toUpperCase()}</div>
                      <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                        {e.sets.map((set,si)=>(
              ...
              return (
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  ...
                  <div style={{fontFamily:F.display,fontSize:15,color:G.teal,letterSpacing:2,marginBottom:6}}>{ex.name.toUpperCase()}</div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {ex.sets.map((set,si)=>(
                      <div key={si} style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"3px 8px",textAlign:"center"}}>
                        <div style={{fontFamily:F.display,fontSize:8,color:G.muted,letterSpacing:1}}>SET {si+1}</div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:G.card,borderTop:`1px solid ${G.border}`,zIndex:301,maxHeight:"88vh",overflowY:"auto",paddingBottom:32}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 20px 16px",borderBottom:`1px solid ${G.border}`}}>
          <div>
            ...
            <div style={{fontFamily:F.display,fontSize:26,color:G.teal,letterSpacing:3}}>LOAD THE BAR</div>
          </div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>
            {(isKg?[60,80,100,120,140,160,180,200]:[135,185,225,275,315,365,405,455]).map(v=>(
              <button key={v} onClick={()=>setInputVal(String(v))} style={{
                background:parseFloat(snapped)===v?G.border:"#0a0a0a",color:parseFloat(snapped)===v?G.teal:G.muted,
                border:`1px solid ${parseFloat(snapped)===v?G.teal:G.border}`,padding:"5px 11px",
            ...
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {[["TOTAL",`${snapped} ${isKg?"kg":"lbs"}`],["BAR",`${BAR_ACTIVE} ${isKg?"kg":"lbs"}`],["PER SIDE",`${perSide} ${isKg?"kg":"lbs"}`]].map(([lbl,val])=>(
                <div key={lbl} style={{flex:1,background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"10px 8px",textAlign:"center"}}>
                  ...
                  <div style={{fontFamily:F.display,fontSize:16,color:G.teal,letterSpacing:1}}>{val}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        <div style={{background:G.card2,padding:"14px 16px",borderBottom:`1px solid ${G.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <span style={{fontFamily:F.display,fontSize:12,color:G.teal,letterSpacing:2,background:`${G.teal}15`,padding:"1px 10px",border:`1px solid ${G.teal}50`}}>{s.focus.toUpperCase()}</span>
        ...
        {!isDone&&<div style={{display:"flex",gap:2,height:3}}>
          {PHASES.map((p,i)=>(
            <div key={p} style={{flex:1,background:phase===p?goldGrad:PHASES.indexOf(phase)>i?`${G.teal}60`:G.border}}/>
          ))}
        </div>}

        {isPR&&phase==="session"&&<div style={{background:"#0a0a0a",padding:"9px 16px",borderBottom:`1px solid ${G.purple}40`}}>
          <span style={{fontFamily:F.display,fontSize:16,color:G.teal,letterSpacing:3}}>🏆 PR WEEK — LEAVE EVERYTHING IN THE TANK</span>
        </div>}
          ...
          <div>
            {readiness&&readiness.adj!==0&&(
              <div style={{padding:"8px 16px",background:G.card2,borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontFamily:F.display,fontSize:14,color:G.teal,letterSpacing:2}}>{readiness.adj}%</span>
              </div>
            )}
            ...
            <div style={{padding:"10px 16px 8px",borderBottom:`1px solid ${G.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                ...
                <span style={{fontFamily:F.display,fontSize:10,color:G.teal,letterSpacing:2}}>{stepIdx+1}/{total}</span>
              </div>
            ...
            {cur.type==="main"&&(
              <div style={{padding:"24px 16px"}}>
                ...
                <div style={{textAlign:"center",marginBottom:20}}>
                  <div style={{fontFamily:F.display,fontSize:11,color:cur.row.isTop?G.teal:G.muted,letterSpacing:4,marginBottom:4}}>
                    {cur.row.ro.toUpperCase()} · {cur.row.lb.toUpperCase()}
                  </div>
                  <div style={{fontFamily:F.display,fontSize:40,color:cur.row.isTop?G.teal:G.text,letterSpacing:3,lineHeight:1,marginBottom:6}}>
                    {cur.ex.name.toUpperCase()}
                  </div>
                  <div style={{fontFamily:F.display,fontSize:72,color:cur.row.isTop?G.teal:G.text,lineHeight:1,marginBottom:4}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:20}}>
                  <button onClick={()=>onTimer({proto:cur.ex.proto,exName:cur.ex.name})}
                    style={{flex:1,padding:"10px 0",background:"#0a0a0a",border:`1px solid ${G.border}`,
                      fontFamily:F.display,fontSize:13,letterSpacing:3,color:G.teal,cursor:"pointer"}}>
                    ▶ {cur.ex.proto} TIMER
                  </button>
                  {cur.row.w&&(
                ...
                <div style={{marginBottom:20}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",background:"#0a0a0a",border:`1px solid ${cur.row.isTop?G.teal:G.border}`,borderRadius:0}}>
                ...
                <div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:20}}>
                  {["i","ii","iii","iv","v"].map((ro,i)=>(
                    <div key={ro} style={{width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",
                      ...
                      background:i<stepIdx-steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main")?goldGrad:i===stepIdx-steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main")?`${G.teal}20`:G.card2,
                      border:`1px solid ${i===stepIdx-steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main")?G.teal:G.border}`,
                      color:i<stepIdx-steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main")?G.black:i===stepIdx-steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main")?G.teal:G.muted}}>
                      {ro}
                    </div>
                  ))}
                </div>
            ...
            {cur.type==="rpe"&&(
              <div style={{padding:"24px 16px"}}>
                ...
                <div style={{textAlign:"center",marginBottom:20}}>
                  ...
                  <div style={{fontFamily:F.display,fontSize:40,color:G.teal,letterSpacing:3,lineHeight:1,marginBottom:8}}>
                    {cur.ex.name.toUpperCase()}
                  </div>
                ...
                <button onClick={()=>onTimer({proto:cur.ex.proto,exName:cur.ex.name})}
                  style={{width:"100%",padding:"10px 0",background:"#0a0a0a",border:`1px solid ${G.border}`,
                    fontFamily:F.display,fontSize:13,letterSpacing:3,color:G.teal,cursor:"pointer",marginBottom:16}}>
                  ▶ {cur.ex.proto} TIMER
                </button>
        ...
        {phase==="complete_acc"&&(
          <div style={{padding:"28px 16px",textAlign:"center"}}>
            ...
            <div style={{fontFamily:F.display,fontSize:40,color:G.teal,letterSpacing:3,lineHeight:1,marginBottom:16}}>SESSION COMPLETE</div>
...
function EMOMTimer({proto,exName,onClose}) {
  ...
  const timeColor=secs>totalSecs*0.6?G.teal:secs>totalSecs*0.3?G.yellow:G.red;
  const R_sm=18,C_sm=2*Math.PI*R_sm;

  return (
    <div style={{position:"fixed",bottom:80,right:12,zIndex:400,background:G.card,
      border:`1px solid ${flash?G.teal:G.border}`,borderTop:`2px solid ${flash?G.teal:G.purple}`,borderRadius:12,minWidth:200,maxWidth:260,
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:"#0a0a0a",borderRadius:"12px 12px 0 0",borderBottom:`1px solid ${G.border}`}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:F.display,fontSize:13,color:G.teal,letterSpacing:3}}>{proto}</div>
      ...
      {minimized&&(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
          ...
          <button onClick={()=>done?reset():setRunning(r=>!r)}
            style={{background:done?goldGrad:running?"#1a0808":`${G.teal}12`,border:"none",borderRadius:6,color:done?G.text:G.teal,padding:"6px 10px",fontSize:14,cursor:"pointer"}}>
            {done?"↺":running?"⏸":"▶"}
          </button>
        </div>
      )}
      {!minimized&&(
        <div style={{padding:"12px 14px"}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
            ...
            <div style={{flex:1}}>
              {done?<div style={{fontSize:13,color:G.teal,fontWeight:800}}>🏆 Done!</div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>done?reset():setRunning(r=>!r)}
              style={{flex:3,padding:"9px",background:done?goldGrad:running?"#1a0808":`${G.teal}12`,
                border:"none",borderRadius:6,color:done?G.text:running?G.red:G.teal,
                fontWeight:800,fontSize:11,letterSpacing:2,cursor:"pointer"}}>
              {done?"START OVER":running?"⏸ PAUSE":"▶ START"}
            </button>
          </div>
          {flash&&<div style={{marginTop:8,textAlign:"center",fontSize:10,color:G.teal,fontWeight:800,letterSpacing:2}}>NEXT ROUND ▶</div>}
        </div>
      )}
    </div>
  );
      ...
      <div style={{minHeight:"100vh",background:"#000000",color:"#F0F0F0",fontFamily:"system-ui,sans-serif",padding:16}}>
        ...
        <div style={{maxWidth:680,margin:"0 auto"}}>
          ...
          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:12}}>PROGRAM PROGRESS</div>
            ...
            <div style={{fontSize:12,color:G.teal,fontWeight:700}}>{completed.size}/16 sessions completed</div>
          </div>

          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:12}}>1RM ({unit})</div>
          ...
          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:12}}>SESSION LOG</div>
                ...
                <div key={s.id} style={{padding:"10px 0",borderBottom:"1px solid #1e1e1e"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:done&&sl?6:0}}>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:10,background:done?`${G.teal}12`:"#1a1a1a",color:done?G.teal:"#2a2a2a",padding:"2px 8px",borderRadius:3,fontWeight:800}}>S{s.id}</span>
                    ...
                    <div key={ex} style={{paddingLeft:12,marginTop:4}}>
                      {entries.map((e,i)=>(
                        <div key={i} style={{fontSize:10,color:"#666666"}}>
                          {ex}: <span style={{color:G.teal,fontWeight:700}}>{e.topWeight} {unit}</span>
                        </div>
                      ))}
                    </div>
                  ))}
    ...
    <div style={{minHeight:"100vh",background:"#000000",color:"#F0F0F0",fontFamily:"system-ui,sans-serif"}}>
      ...
      <header style={{background:"rgba(8,8,8,0.97)",borderBottom:"1px solid #2a2a2a",padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontSize:13,fontWeight:800,color:G.teal}}>{profile.name}</div>
        </div>
      </header>
      <main style={{maxWidth:680,margin:"0 auto",padding:"16px 14px"}}>
        <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:14}}>
        ...
        {athletes.map(a=>{
          return (
            <button key={a.id} onClick={()=>selectAthlete(a)}
              ...
              <span style={{color:G.teal,fontSize:18}}>›</span>
            </button>
          );
        })}
      ...
      <div style={{...cs,padding:16}}>
        ...
        {loading2 && parseFloat(inputVal)!==parseFloat(dispSnapped) && (
          <div style={{marginTop:8,fontSize:11,color:G.muted,textAlign:"center",fontFamily:F.body}}>
            Snapped to <span style={{color:G.teal,fontWeight:700}}>{dispSnapped} {blUnit}</span>
          </div>
        )}
        {!loading2 && inputVal && (
          <div style={{marginTop:8,fontSize:11,color:G.teal,textAlign:"center",fontFamily:F.body}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          {[["TOTAL",`${snapped.toFixed(1)} kg`,`${(snapped*2.20462).toFixed(1)} lbs`],
            ...
            <div key={lbl} style={{background:G.card,border:`1px solid ${G.border}`,padding:"12px 8px",textAlign:"center"}}>
              ...
              <div style={{fontFamily:F.display,fontSize:18,color:G.teal,letterSpacing:1}}>{val}</div>
    ...
    <div style={{minHeight:"100vh",background:G.black,color:G.text,fontFamily:F.body,paddingBottom:72}}>
      ...
      <header style={{background:"rgba(8,8,8,0.98)",borderBottom:`1px solid ${G.border}`,padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:3,zIndex:50}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontFamily:F.display,fontSize:18,color:G.teal,letterSpacing:2,lineHeight:1}}>{profile.name}</div>
        </div>
      </header>
          ...
          <div>
            <div style={{...cardStyle,padding:16,marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                ...
                <span style={{fontSize:12,color:G.teal,fontWeight:800}}>{completed.size}/16 SESSIONS</span>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                {PROGRAM.map(w=>{const wd=w.sessions.filter(s=>completed.has(s.id)).length;return(
                  <div key={w.week} style={{textAlign:"center",background:"#0a0a0a",borderRadius:8,padding:"10px 4px"}}>
                    <div style={{fontSize:18,fontWeight:900,color:wd===4?G.teal:wd>0?G.teal:"#2a2a2a"}}>{wd}/4</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
              {[["Done",completed.size],["Left",16-completed.size],["Week",Math.min(4,Math.ceil(completed.size/4)+1)]].map(([l,v])=>(
                <div key={l} style={{...cardStyle,padding:"14px 8px",textAlign:"center"}}>
                  <div style={{fontFamily:F.display,fontSize:42,color:G.teal,lineHeight:1}}>{v}</div>
            ...
            {nextSession&&(
              <div style={{marginBottom:18}}>
                <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:8}}>NEXT SESSION</div>
                <div onClick={()=>setModal(nextSession)}
                  style={{...cardStyle,padding:"14px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",border:`1px solid ${G.teal}30`}}>
                  <div>
                    <div style={{fontWeight:800,fontSize:15,color:G.text}}>Session {nextSession.id} -- {nextSession.day}</div>
                    <div style={{fontSize:11,color:FC[nextSession.focus]||G.teal,marginTop:4,letterSpacing:1}}>{nextSession.focus}</div>
                    <div style={{display:"flex",gap:5,marginTop:8,flexWrap:"wrap"}}>
                      {flatEx(nextSession.exercises).map((e,i)=>(
                        <span key={i} style={{fontSize:10,color:e.type==="main"?G.teal:G.muted,background:e.type==="main"?`${G.teal}18`:`${G.purple}18`,borderRadius:3,padding:"2px 7px"}}>{e.name}</span>
                      ))}
                    </div>
                  </div>
                  <span style={{fontSize:22,color:G.teal,flexShrink:0,marginLeft:10}}>▶</span>
                </div>
              </div>
            )}
            {completed.size===16&&(
              <div style={{...cardStyle,padding:24,textAlign:"center",border:`1px solid ${G.teal}40`,marginBottom:18}}>
                <div style={{fontSize:40,marginBottom:8}}>🏆</div>
                <div style={{fontSize:18,fontWeight:900,color:G.teal,letterSpacing:2}}>PROGRAM COMPLETE</div>
                <div style={{fontSize:11,color:G.muted,marginTop:6}}>You've peaked. Go hit those PRs.</div>
              </div>
            )}
            <div>
              <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:8}}>1RM BOARD</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
                {LIFTS.map(l=>(
                  <div key={l} style={{...card2Style,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <span style={{fontSize:14,fontWeight:800,color:rms[l]?G.teal:"#2a2a2a"}}>{rms[l]||"--"} <span style={{fontSize:9,color:G.muted}}>{rms[l]?unit:""}</span></span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              {PROGRAM.map(w=>{const wd=w.sessions.filter(s=>completed.has(s.id)).length,act=activeWeek===w.week;return(
                <button key={w.week} onClick={()=>setActiveWeek(w.week)}
                  style={{flex:1,padding:"10px 4px",borderRadius:8,border:act?`1px solid ${G.teal}`:`1px solid ${G.border}`,
                    background:act?`${G.teal}15`:"#0a0a0a",color:act?G.teal:G.muted,cursor:"pointer"}}>
                  <div style={{fontWeight:900,fontSize:13,letterSpacing:1}}>WK {w.week}</div>
                  <div style={{fontSize:10,color:act?G.teal:"#2a2a2a",marginTop:2}}>{wd}/4</div>
                </button>
              );})}
            </div>
            <div style={{padding:"8px 14px",background:`${G.teal}10`,borderLeft:`3px solid ${G.teal}`,borderRadius:"0 4px 4px 0",marginBottom:14,display:"flex",gap:12,alignItems:"center"}}>
              <span style={{fontSize:11,color:G.teal,fontWeight:800,letterSpacing:2}}>WEEK {activeWeek} -- {weekData.theme.toUpperCase()}</span>
              <span style={{fontSize:11,color:G.muted}}>{weekData.range}</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {weekData.sessions.map(s=>{
                const isDone=completed.has(s.id),fc=FC[s.focus]||G.teal;
                const mainEx=s.exercises.filter(e=>e.type==="main"),accEx=s.exercises.filter(e=>e.type!=="main");
                return(
                  <div key={s.id} style={{...cardStyle,padding:"13px 14px",borderTop:`1px solid ${G.teal}`,borderLeft:"none",borderRight:"none",borderBottom:`1px solid ${G.border}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
                          ...
                          {isDone&&<span style={{fontSize:10,color:G.teal,fontWeight:700}}>✓ DONE</span>}
                        </div>
                        {mainEx.length>0&&<div style={{marginBottom:8}}>
                          <div style={{fontSize:8,color:G.teal,letterSpacing:2,fontWeight:700,marginBottom:5}}>MAIN LIFTS</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {mainEx.map((e,i)=><span key={i} style={{fontSize:11,color:G.teal,background:`${G.teal}15`,borderRadius:3,padding:"2px 8px"}}>{e.name}</span>)}
                          </div>
                        </div>}
                        {accEx.length>0&&<div>
                          <div style={{fontSize:8,color:`${G.teal}20`,letterSpacing:2,fontWeight:700,marginBottom:5}}>ACCESSORY</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {accEx.map((e,i)=>e.type==="superset"
                              ?<span key={i} style={{fontSize:11,color:G.teal,background:"#0f0f0f",borderRadius:3,padding:"2px 8px"}}>{e.exA.name}+{e.exB.name}</span>
                              :<span key={i} style={{fontSize:11,color:G.muted,background:`${G.purple}18`,borderRadius:3,padding:"2px 8px"}}>{e.name}</span>
                            )}
                          </div>
                        </div>}
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
                        <button onClick={()=>setModal(s)}
                          style={{background:"#111",border:`1px solid ${G.border}`,borderRadius:6,color:G.teal,padding:"7px 12px",fontSize:10,fontWeight:700,letterSpacing:1,cursor:"pointer"}}>
                          VIEW
                        </button>
                        <button onClick={()=>toggleDone(s.id)}
                          style={{background:isDone?"#0a0a0a":"#0a0a0a",border:isDone?`1px solid ${G.teal}40`:`1px solid ${G.border}`,
                            borderRadius:6,color:isDone?G.teal:G.muted,padding:"7px 12px",fontSize:10,fontWeight:700,letterSpacing:1,cursor:"pointer"}}>
                          {isDone?"✓":"DONE?"}
                        </button>
                      </div>
                    </div>
            ...
            {completed.size===0?(
              ...
              return(
                <div key={w.week} style={{marginBottom:20}}>
                  <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3,marginBottom:10}}>
                    WEEK {w.week} -- {w.theme.toUpperCase()}
                  </div>
                  {ws.map(s=>{
                    const fc=FC[s.focus]||G.teal,sl=logs[s.id],al=accLogs[s.id];
                    return(
                      <div key={s.id} style={{background:G.card,borderRadius:8,border:`1px solid ${G.teal}30`,marginBottom:8}}>
                        <div style={{padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          ...
                          <span style={{color:G.teal,fontSize:20}}>✓</span>
                        </div>
                          ...
                          <div style={{padding:"8px 14px",borderTop:`1px solid ${G.border}`,background:"#0a0a0a"}}>
                            {sl&&Object.entries(sl).map(([en,entries])=>(
                              <div key={en} style={{marginBottom:6}}>
                                <div style={{fontSize:10,color:G.teal,fontWeight:700,marginBottom:3}}>{en}</div>
                            ...
                            {al&&Object.entries(al).map(([en,sets])=>{
                              ...
                              return <div key={en} style={{marginBottom:6}}><div style={{fontSize:10,color:G.teal,fontWeight:700,marginBottom:3}}>{en}</div><div style={{fontSize:11,color:G.muted}}>{weights.join(", ")} {unit}</div></div>;
                            })}
                          </div>
                        ):null}
                      </div>
        ...
        {tab==="about"&&(
          <div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.teal}>PROGRAM OVERVIEW</SH>
              <div style={{fontSize:13,color:G.text,lineHeight:1.8}}>
                ...
                {[
                  ...
                  <div key={title} style={{marginBottom:12,padding:"10px 12px",background:G.card2,borderRadius:8,border:`1px solid ${G.border}`}}>
                    <div style={{fontWeight:800,color:G.teal,fontSize:12,marginBottom:4}}>{title}</div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.teal}>HOW TO USE THIS APP</SH>
              {[
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.teal}>SET NOTATION</SH>
              {[
                ...
                <div key={term} style={{display:"flex",gap:10,marginBottom:10,padding:"8px 10px",background:G.card2,borderRadius:6,border:`1px solid ${G.border}`}}>
                  <span style={{fontWeight:800,color:G.teal,fontSize:11,minWidth:80}}>{term}</span>
            ...
            <div style={{...cardStyle,padding:"20px 16px",marginBottom:14,textAlign:"center"}}>
              <SH color={G.teal}>SUPPORT THE PROJECT</SH>
        ...
        {tab==="profile"&&(
          <div>
            <div style={{...cardStyle,padding:"20px 16px",marginBottom:14,textAlign:"center"}}>
              ...
              <div style={{marginTop:10,fontSize:11,color:G.teal,fontWeight:700}}>{completed.size}/16 sessions completed</div>
            </div>
            ...
            <div style={{...cardStyle,padding:16,marginBottom:14}}>
              <div style={{fontFamily:F.display,fontSize:16,color:G.teal,letterSpacing:4,marginBottom:14}}>1RM SETTINGS</div>
                  ...
                  {rm>0&&<div style={{marginTop:7,display:"flex",gap:8,flexWrap:"wrap"}}>
                    {[[0.70,"70%"],[0.85,"85%"],[0.88,"88%"],[0.91,"91%"]].map(([p,lbl])=>(
                      <span key={lbl} style={{fontSize:10,color:G.muted,background:"#0f0f0f",border:`1px solid ${G.border}`,borderRadius:3,padding:"3px 8px"}}>
                        <span style={{color:G.teal,fontWeight:700}}>{r25(rm*p, unit)}</span> @ {lbl}
                      </span>
                    ))}
                  </div>}
                </div>
            ...
            <div style={{...cardStyle,padding:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div style={{fontSize:10,color:G.teal,fontWeight:800,letterSpacing:3}}>MEV/MRV FACTORS</div>
                <div style={{fontSize:12,fontWeight:800,color:volScore>0?G.teal:volScore<0?G.red:G.muted}}>{volScore>0?"+":""}{volScore}</div>
              </div>
        ...
        {NAV.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{flex:1,padding:"10px 4px",background:"transparent",border:"none",
              color:tab===t.id?G.teal:G.muted,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,
              borderTop:tab===t.id?`2px solid ${G.purple}`:"2px solid transparent",marginTop:-2}}>
            <span style={{fontSize:15}}>{t.icon}</span>
            <span style={{fontFamily:F.display,fontSize:9,letterSpacing:2,color:tab===t.id?G.teal:G.muted}}>{t.label.toUpperCase()}</span>
          </button>
        ))}
      </nav>
