...
function MainBlock({ex,rms,unit,sid,onLog,logs,onTimer}) {
  ...
  const top=valid?r25(rm*topPct, unit):null;
  ...
  return (
    <div style={{border:nested?"none":`1px solid #1a3050`,borderRadius:nested?0:8,overflow:"hidden"}}>
      <div style={{background:nested?"#040d18":"#060e1a",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:F.display,fontSize:20,color:G.blue,letterSpacing:2,lineHeight:1}}>{ex.name}</span>
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,borderRadius:12,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        <div style={{padding:"12px 16px",borderTop:`1px solid ${G.border}`}}>
          <button onClick={onToggle}
            style={{width:"100%",padding:14,background:isDone?`linear-gradient(90deg,#0a2a0a,#0f3a0f)`:goldGrad,
              border:"none",color:isDone?G.green:G.black,
                ...
                <div key={s.id} style={{padding:"10px 0",borderBottom:"1px solid #1e1e1e"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:done&&sl?6:0}}>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:10,background:done?"#0a2a0a":"#1a1a1a",color:done?"#22c55e":"#2a2a2a",padding:"2px 8px",borderRadius:3,fontWeight:800}}>S{s.id}</span>
      ...
      <main style={{maxWidth:680,margin:"0 auto",padding:"16px 14px"}}>
        ...
        {tab==="about"&&(
          <div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.blue}>HOW TO USE THIS APP</SH>
              {[
