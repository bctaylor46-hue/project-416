...
const G = {
  // GSF platform colors
  gold:"#C9A84C",   goldLt:"#e0c068",  goldDim:"#8a6e28",
  purple:"#6B3080", teal:"#2A7A78",
  black:"#000000",  card:"#111111",     card2:"#1a1a1a",
  border:"#2a2a2a", border2:"#1e1e1e", text:"#F0F0F0",  muted:"#666666",
  red:"#ef4444",    yellow:"#eab308",
};
// GSF primary gradient: purple → teal
const goldGrad = `linear-gradient(90deg, ${G.purple}, ${G.teal})`;
// Accent gradient for highlights (reversed)
const accentGrad = `linear-gradient(90deg, ${G.teal}, ${G.purple})`;
...
const rpeClr = v => v >= 10 ? G.red : v >= 8 ? "#f97316" : v >= 6 ? G.yellow : G.muted;
...
const FC    = {"Upper Body":G.text,"Lower Body":G.text,"Full Body":G.text};
...
const ST = {
  ...
  saveProgress: async (uid, completed, logs, accLogs, rms) => {
    await Promise.all([
      ...
      rms && Object.keys(rms).length > 0
        ? supabase.from('profiles').update({ rms }).eq('id', uid)
        : Promise.resolve(),
    ]);
  },
...
const FBtn = ({children,onClick,secondary,disabled}) => (
  <button onClick={onClick} disabled={!!disabled}
    style={{flex:1,padding:"12px 16px",background:disabled?"#1a1a1a":secondary?"transparent":goldGrad,
      color:disabled?"#333":secondary?G.muted:G.text,
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
      {f.opts.map(o => { const sel = mev[f.key]===o.v; return (
        <button key={o.l} onClick={()=>tog(f.key,o.v)}
          style={{padding:"5px 11px",borderRadius:4,border:sel?`1px solid ${G.border}`:`1px solid ${G.border}`,
            background:sel?G.card2:"#0a0a0a",color:sel?G.text:G.muted,fontSize:11,cursor:"pointer",fontWeight:sel?700:400}}>
          {o.l} <span style={{fontSize:9,color:sel?G.muted:"#333"}}>({o.v>0?"+":""}{o.v})</span>
        </button>
      );})}
    </div>
  </div>
  ...
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <span style={{fontFamily:F.display,fontSize:14,color:val?G.text:G.muted,letterSpacing:1}}>
          {val ? `${score>0?"+":""}${score}` : "—"}
        </span>
      </div>
...
const SH = ({children,color}) => (
  <div style={{fontFamily:F.display,fontSize:13,color:color||G.text,letterSpacing:4,marginBottom:10,paddingBottom:8,borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:10}}>
    {children}
  </div>
);
...
const Logo = ({size="lg"}) => {
  ...
  return (
    <div style={{textAlign:"center",fontFamily:F.body}}>
      ...
      <div style={{fontFamily:F.display,fontSize:big?72:26,color:G.text,lineHeight:1,letterSpacing:big?3:1}}>
        4:16
      </div>
      <div style={{height:2,width:big?80:30,background:goldGrad,margin:big?"4px auto 0":"2px auto 0"}}/>
...
function Onboarding({onComplete}) {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 16px",background:G.black,position:"relative",overflow:"hidden"}}>
      <div style={{position:"fixed",inset:0,zIndex:0,background:"radial-gradient(ellipse at 20% 50%, rgba(107,48,128,0.12) 0%, transparent 60%), radial-gradient(ellipse at 80% 50%, rgba(42,122,120,0.12) 0%, transparent 60%)"}}/>
          ...
          <div style={{display:"flex",gap:6,marginBottom:22}}>
            {OB_STEPS.map((s,i)=>(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                <div style={{width:"100%",height:2,borderRadius:99,background:i<step?G.text:i===step?G.text:"#2a2a2a"}}/>
                <span style={{fontSize:8,color:i===step?G.text:i<step?G.muted:"#2a2a2a",fontWeight:700,letterSpacing:1}}>{s.toUpperCase()}</span>
              </div>
            ))}
          </div>
        )}
        <div style={card}>
              ...
              <div style={{display:"flex",marginBottom:20,background:"#0a0a0a",borderRadius:6,overflow:"hidden"}}>
                {[["signup","SIGN UP"],["signin","SIGN IN"]].map(([k,l])=>(
                  <button key={k} onClick={()=>{setMode(k);setErr("");}}
                    style={{flex:1,padding:11,background:mode===k?goldGrad:"transparent",
                      color:mode===k?G.text:G.muted,border:"none",fontWeight:800,fontSize:11,letterSpacing:2,cursor:"pointer"}}>
                    {l}
                  </button>
                ))}
              </div>
          ...
          {step===2 && (
            <div>
              ...
              <div style={{background:"#0f0f0f",border:`1px solid ${G.border}`,borderRadius:8,padding:"12px 14px",marginBottom:16}}>
                <div style={{fontSize:9,color:G.muted,marginBottom:4,letterSpacing:2}}>YOUR MEV/MRV MODIFIER</div>
                <div style={{fontSize:28,fontWeight:900,color:G.text}}>
                  {mevScore>0?"+":""}{mevScore}
                </div>
              ...
              <div style={{display:"flex",background:"#0a0a0a",borderRadius:6,overflow:"hidden",marginBottom:14}}>
                {["lbs","kg"].map(u=>(
                  <button key={u} onClick={()=>setUnit(u)}
                    style={{flex:1,padding:9,background:unit===u?goldGrad:"transparent",
                      color:unit===u?G.text:G.muted,border:"none",fontWeight:800,fontSize:11,letterSpacing:2,cursor:"pointer"}}>
                    {u.toUpperCase()}
                  </button>
                ))}
              </div>
                ...
                <div key={l} style={{marginBottom:12}}>
                  ...
                  <div style={{display:"flex",background:"#111",borderRadius:6,overflow:"hidden",border:`1px solid ${G.border}`}}>
                    <input type="number" value={rms[l]||""} placeholder="Leave blank if unknown"
                      onChange={e=>{const v=e.target.value;setRms(p=>({...p,[l]:v}));}}
                      style={{flex:1,background:"transparent",border:"none",color:G.text,padding:"10px 12px",fontSize:13,outline:"none"}}/>
    ...
    <div>
      <div style={{background:G.card,borderRadius:10,padding:"12px 16px",border:`1px solid ${G.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          ...
          <div style={{fontSize:26,fontWeight:900,color:G.text}}>{volScore>0?"+":""}{volScore}</div>
        </div>
        <button onClick={()=>setShowRef(s=>!s)}
          style={{background:showRef?G.card2:"#0a0a0a",border:`1px solid ${showRef?"#2a2a2a":G.border}`,
            borderRadius:6,color:showRef?G.text:G.muted,padding:"8px 14px",fontSize:10,fontWeight:700,letterSpacing:2,cursor:"pointer"}}>
            ...
            <thead>
              <tr style={{background:"#111111"}}>
                {["Body Part","MV","MEV","MAV","MRV","Freq","Rep Range"].map(h=>(
                  <th key={h} style={{padding:"6px 8px",fontSize:9,color:G.muted,fontWeight:800,letterSpacing:1,textAlign:"left"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {BODY_REF.map(([name,...vals],i)=>(
                <tr key={name} style={{background:i%2?"#111111":"transparent"}}>
                  <td style={{padding:"6px 8px",fontSize:11,fontWeight:700,color:G.text}}>{name}</td>
      ...
      <div style={{display:"flex",gap:7,marginBottom:14}}>
        {PROGRAM.map(w=>{const act=activeWeek===w.week;return(
          <button key={w.week} onClick={()=>setActiveWeek(w.week)}
            style={{flex:1,padding:"8px 4px",borderRadius:7,border:`1px solid ${G.border}`,
              background:act?G.card2:"#0a0a0a",color:act?G.text:G.muted,fontFamily:F.display,fontSize:15,cursor:"pointer",letterSpacing:3}}>
            WK {w.week}
          </button>
        );})}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {PATS.map(pat=>{
          ...
          const SI={none:{l:"No Volume",c:"#333"},below:{l:"Below MEV",c:G.red},good:{l:"In Range",c:G.text},near:{l:"Near MRV",c:G.muted},over:{l:"Over MRV",c:G.red}}[status];
          const bcLine={none:"#333",below:G.red,good:G.text,near:G.muted,over:G.red}[status];
          const patEx=[];
          weekData.sessions.forEach(s=>s.exercises.forEach(ex=>{
          ...
          return(
            <div key={pat.id} style={{background:G.card,borderRadius:10,border:`1px solid ${G.border}`,padding:14}}>
              ...
              <div style={{background:"#1a1a1a",borderRadius:99,height:6,marginBottom:7,position:"relative",overflow:"visible"}}>
                ...
                {adjMev>0&&<div style={{position:"absolute",top:-3,left:`${Math.min(99,(adjMev/adjMrv)*100)}%`,width:1,height:12,background:G.muted,opacity:0.4}}/>}
              </div>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <span style={{fontSize:9,color:G.muted}}>MEV: {adjMev}</span>
                <span style={{fontSize:9,color:SI.c,fontWeight:800}}>{SI.l}</span>
                <span style={{fontSize:9,color:G.muted}}>MRV: {adjMrv}</span>
              </div>
              {patEx.length>0
...
const ProtoTag = ({proto,exName,onTimer}) => (
  <div style={{display:"flex",gap:5,alignItems:"center"}}>
    <span style={{fontFamily:F.display,fontSize:12,background:G.card2,color:G.muted,padding:"1px 8px",letterSpacing:2}}>{proto}</span>
    <button onClick={()=>onTimer({proto,exName})}
      style={{background:"#0a0a0a",border:`1px solid ${G.border}`,borderRadius:3,color:G.muted,padding:"2px 7px",fontSize:9,cursor:"pointer",letterSpacing:1,fontWeight:700}}>
      ▶ TIMER
    </button>
  </div>
);
const ProtoTagAcc = ({proto,exName,onTimer}) => (
  <div style={{display:"flex",gap:5,alignItems:"center",flexShrink:0}}>
    {proto&&<span style={{fontSize:10,color:G.muted,background:G.card2,borderRadius:3,padding:"2px 7px",fontWeight:700,letterSpacing:1}}>{proto}</span>}
...
function MainBlock({ex,rms,unit,sid,onLog,logs,onTimer,readinessAdj=0}) {
  ...
  const ROWS=[
    {ro:"i",  lb:"Build 1", reps:rp2[0],w:W[0],tag:"70% of top",  isTop:false},
    {ro:"ii", lb:"Build 2", reps:rp2[1],w:W[1],tag:"85% of top",  isTop:false},
  ...
  return (
    <div style={{border:`1px solid ${G.border}`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:G.card2,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <span style={{fontFamily:F.display,fontSize:20,color:G.text,letterSpacing:2,lineHeight:1}}>{ex.name}</span>
      ...
      <div style={{display:"flex",alignItems:"center",background:"#111111",padding:"6px 14px"}}>
        ...
        <div style={{width:72,textAlign:"right",fontSize:9,color:G.muted,fontWeight:700,letterSpacing:1,flexShrink:0}}>WEIGHT</div>
      </div>
      {ROWS.map(row=>(
        <div key={row.ro} style={{display:"flex",alignItems:"center",borderTop:`1px solid ${G.border}`,padding:"9px 14px",background:row.isTop?"#1a1a1a":"transparent"}}>
          <div style={{width:24,flexShrink:0}}><span style={{fontSize:10,color:G.muted,fontWeight:700}}>{row.ro}</span></div>
          <div style={{flex:"1 1 auto",fontSize:12,color:row.isTop?G.text:G.muted,fontWeight:row.isTop?700:400}}>{row.lb}</div>
          <div style={{width:34,textAlign:"center",fontSize:15,fontWeight:900,color:G.text,flexShrink:0}}>{row.reps}</div>
          <div style={{width:96,textAlign:"center",fontSize:10,color:"#444",flexShrink:0}}>{row.tag}</div>
          <div style={{width:72,textAlign:"right",flexShrink:0,fontSize:row.isTop?17:13,fontWeight:row.isTop?900:600,color:G.text}}>
            {row.w!=null?<>{row.w}<span style={{fontSize:9,color:G.muted,fontWeight:400,marginLeft:3}}>{unit}</span></>:<span style={{color:"#2a2a2a"}}>--</span>}
          </div>
        </div>
      ))}
      <div style={{padding:"9px 14px",borderTop:`1px solid ${G.border}`,background:"#111111"}}>
        {top
          ?<button onClick={()=>{onLog(sid,ex.name,{topWeight:top});setLogged(true);}} disabled={logged}
            style={{padding:"6px 14px",background:logged?G.card2:"#0f0f0f",border:`1px solid ${G.border}`,
              borderRadius:4,color:logged?G.text:G.muted,fontSize:10,fontWeight:700,letterSpacing:2,cursor:logged?"default":"pointer"}}>
...
function RpeBlock({ex,sid,accLogs,setAccLog,unit,onTimer,nested=false}) {
  ...
  return (
    <div style={{border:nested?"none":`1px solid ${G.border}`,borderRadius:nested?0:8,overflow:"hidden"}}>
      <div style={{background:nested?"#0f0f0f":"#0f0f0f",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:F.display,fontSize:20,color:G.text,letterSpacing:2,lineHeight:1}}>{ex.name}</span>
        {ex.proto&&<ProtoTagAcc proto={ex.proto} exName={ex.name} onTimer={onTimer}/>}
      </div>
      <div style={{display:"flex",alignItems:"center",background:"#111111",padding:"5px 14px"}}>
        <div style={{width:24,flexShrink:0}}/>
        <div style={{flex:"1 1 auto",fontSize:9,color:"#2a2a2a",fontWeight:700,letterSpacing:1}}>SET</div>
        <div style={{width:34,textAlign:"center",fontSize:9,color:"#2a2a2a",fontWeight:700,flexShrink:0}}>REPS</div>
        <div style={{width:82,textAlign:"center",fontSize:9,color:"#2a2a2a",fontWeight:700,letterSpacing:1,flexShrink:0}}>RPE</div>
        <div style={{width:80,textAlign:"right",fontSize:9,color:G.muted,fontWeight:700,letterSpacing:1,flexShrink:0}}>WEIGHT</div>
      </div>
      {ex.sets.map((set,i)=>{
        const wt=exLogs[i]||"";
        return (
          <div key={i} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:`1px solid ${G.border}`}}>
            <div style={{width:24,flexShrink:0}}><span style={{fontSize:10,color:G.muted,fontWeight:700}}>{i+1}</span></div>
            ...
            <div style={{width:82,textAlign:"center",flexShrink:0}}>
              {set.rpe!=null
                ...
                :<span style={{fontSize:10,color:G.muted,fontStyle:"italic",whiteSpace:"nowrap"}}>{set.note||"= prev"}</span>
              }
            </div>
            <div style={{width:80,textAlign:"right",flexShrink:0}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:3}}>
                <input type="number" value={wt} onChange={e=>setAccLog(sid,ex.name,i,e.target.value)}
                  style={{width:46,background:"#0a0a0a",border:`1px solid ${G.border}`,borderRadius:4,
...
function SupersetBlock({ex,sid,accLogs,setAccLog,unit,onTimer}) {
  return (
    <div style={{border:`1px solid #2a2a2a40`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:"#0f0f0f",padding:"7px 14px",borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:9,color:G.muted,fontWeight:900,letterSpacing:2,background:G.card,borderRadius:3,padding:"2px 8px"}}>SUPERSET</span>
        <span style={{fontSize:10,color:G.muted}}>{ex.exA.name} + {ex.exB.name}</span>
      </div>
      <RpeBlock ex={ex.exA} sid={sid} accLogs={accLogs} setAccLog={setAccLog} unit={unit} onTimer={onTimer} nested/>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"4px",background:"#111111",borderTop:`1px solid ${G.border}`,borderBottom:`1px solid ${G.border}`}}>
        <span style={{fontSize:9,color:"#444444",fontWeight:700,letterSpacing:2}}>⟷ SUPERSET ⟷</span>
      </div>
...
function ReadinessGate({onComplete}) {
  ...
  return (
    <div style={{padding:"20px 16px"}}>
      ...
      <div style={{fontFamily:F.display,fontSize:28,color:G.text,letterSpacing:3,marginBottom:6,lineHeight:1}}>HOW ARE YOU SHOWING UP?</div>
      ...
      {READINESS_Qs.map(q=>{
        ...
        const c = q.inverted
          ? (val<=4?G.text:val<=7?G.muted:G.muted)
          : (val>=7?G.text:val>=4?G.muted:G.muted);
        return (
          <div key={q.id} style={{marginBottom:20}}>
            ...
            <input type="range" min={1} max={10} value={val}
              ...
              style={{width:"100%",WebkitAppearance:"none",appearance:"none",height:3,borderRadius:2,
                background:`linear-gradient(90deg, ${G.text} ${pct}%, #2a2a2a ${pct}%)`,outline:"none",cursor:"pointer"}}/>
          </div>
        );
      })}
      ...
      <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"14px 16px",marginBottom:20,marginTop:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
          ...
          <span style={{fontFamily:F.display,fontSize:20,color:G.text,letterSpacing:3}}>{prof.label}</span>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,padding:"0 4px"}}>
        ...
        <span style={{fontFamily:F.display,fontSize:28,color:G.text,letterSpacing:1}}>{score}/10</span>
        <div style={{flex:1,height:1,background:G.border}}/>
      </div>

      <button onClick={()=>onComplete({score,profile:prof,adj:prof.adj})}
        style={{width:"100%",padding:14,background:goldGrad,border:"none",
          fontFamily:F.display,fontSize:20,letterSpacing:4,color:G.text,cursor:"pointer"}}>
        LOAD MY SESSION →
      </button>
    </div>
  );
...
const qColor = v => v>=8?G.text:v>=6?G.text:v>=4?G.muted:"#444";
...
function WinsScreen({readiness,sessionId,completed,qualityRatings,onClose}) {
  ...
  return (
    <div style={{padding:"28px 16px",textAlign:"center"}}>
      ...
      <div style={{fontFamily:F.display,fontSize:48,color:G.text,letterSpacing:3,lineHeight:1,marginBottom:6}}>YOU SHOWED UP.</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
        {[
          ...
          <div key={label} style={{background:G.card2,border:`1px solid ${G.border}`,padding:"16px 10px"}}>
            <div style={{fontSize:20,marginBottom:6}}>{icon}</div>
            <div style={{fontFamily:F.display,fontSize:22,color:G.text,letterSpacing:2,lineHeight:1}}>{val}</div>
      ...
      {hasRatings&&(
        <div style={{background:G.card,border:`1px solid ${G.border}`,overflow:"hidden",marginBottom:20,textAlign:"left"}}>
          ...
          {Object.entries(qualityRatings).filter(([,r])=>r.length>0).map(([name,ratings])=>{
            ...
            return (
              <div key={name} style={{padding:"12px 16px",borderBottom:`1px solid ${G.border}`}}>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div id="session-share-card" style={{
          background:G.black,border:`1px solid ${G.border}`,padding:"24px 20px",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${G.border}`}}>
            <div>
              ...
              <div style={{fontFamily:F.display,fontSize:32,color:G.text,lineHeight:1,letterSpacing:2}}>4:16</div>
              <div style={{fontFamily:F.display,fontSize:7,color:G.muted,letterSpacing:4,marginTop:2}}>LIFTERS ON THE RUN</div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontFamily:F.display,fontSize:22,color:G.text,letterSpacing:2,lineHeight:1}}>S{sessionId}</div>
          ...
          <div style={{display:"flex",gap:16,marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${G.border}`}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontFamily:F.display,fontSize:28,color:G.text,letterSpacing:2,lineHeight:1}}>{readiness.score}<span style={{fontSize:14,color:G.muted}}>/10</span></div>
              <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:2,marginTop:3}}>{readiness.profile.icon} {readiness.profile.label}</div>
            </div>
            <div style={{flex:2}}>
              <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:6}}>PROGRAM</div>
              <div style={{fontFamily:F.display,fontSize:22,color:G.text,letterSpacing:2,lineHeight:1,marginBottom:6}}>{completed.size}<span style={{fontSize:12,color:G.muted}}>/16 SESSIONS</span></div>
              <div style={{height:4,background:"#1a1a1a",marginBottom:3}}><div style={{height:4,background:goldGrad,width:`${(completed.size/16)*100}%`}}/></div>
            </div>
          </div>

          {/* Full workout — every exercise */}
          {sess&&sess.exercises.map((ex,i)=>{
            if (ex.type==="main") {
              ...
              return (
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div>
                      <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:2}}>MAIN LIFT · {ex.proto}</div>
                      <div style={{fontFamily:F.display,fontSize:18,color:G.text,letterSpacing:2,lineHeight:1}}>{ex.name.toUpperCase()}</div>
                    </div>
                    {avg!==null&&<div style={{textAlign:"right"}}>
                      ...
                      <div style={{fontFamily:F.body,fontSize:8,color:qColor(avg),textTransform:"uppercase",letterSpacing:1}}>{QUALITY[Math.round(avg)-1]?.label}</div>
                    </div>}
                  </div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {["i","ii","iii","iv","v"].map((ro,ri)=>(
                      <div key={ro} style={{background:"#0a0a0a",border:`1px solid ${ri===2?"#555":G.border}`,padding:"4px 8px",textAlign:"center",minWidth:44}}>
                        <div style={{fontFamily:F.display,fontSize:8,color:ri===2?G.text:G.muted,letterSpacing:1}}>{ro}</div>
                        <div style={{fontFamily:F.display,fontSize:13,color:G.text,letterSpacing:1}}>{ex.pat?.reps[ri]}</div>
                      </div>
                    ))}
                    <div style={{background:G.card,border:`1px solid ${G.border}`,padding:"4px 8px",textAlign:"center",alignSelf:"center"}}>
                      <div style={{fontFamily:F.display,fontSize:8,color:G.muted,letterSpacing:1}}>TARGET</div>
                      <div style={{fontFamily:F.display,fontSize:13,color:G.muted,letterSpacing:1}}>{ex.pat?.tl}{readiness.adj?` ${readiness.adj}%ADJ`:""}</div>
                    </div>
                  </div>
                </div>
              );
              ...
              return (
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:6}}>SUPERSET · {ex.exA.proto}</div>
                  {[ex.exA, ex.exB].map((e,j)=>(
                    <div key={j} style={{marginBottom:6}}>
                      <div style={{fontFamily:F.display,fontSize:15,color:G.muted,letterSpacing:2,marginBottom:3}}>{e.name.toUpperCase()}</div>
                      <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                        {e.sets.map((set,si)=>(
                          <div key={si} style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"3px 8px",textAlign:"center"}}>
                            <div style={{fontFamily:F.display,fontSize:8,color:G.muted,letterSpacing:1}}>SET {si+1}</div>
                            <div style={{fontFamily:F.display,fontSize:12,color:G.text,letterSpacing:1}}>{set.n>1?`${set.n}×`:""}{set.reps}{set.rpe?` @${set.rpe}`:""}</div>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              return (
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:4}}>{ex.proto}</div>
                  <div style={{fontFamily:F.display,fontSize:15,color:G.muted,letterSpacing:2,marginBottom:6}}>{ex.name.toUpperCase()}</div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {ex.sets.map((set,si)=>(
                      <div key={si} style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"3px 8px",textAlign:"center"}}>
                        <div style={{fontFamily:F.display,fontSize:8,color:G.muted,letterSpacing:1}}>SET {si+1}</div>
...
function BarLoaderSheet({seedWeight, userUnit, onClose}) {
  const isKg = userUnit === "kg";
  const [inputVal, setInputVal] = useState(String(Math.round(seedWeight)));
  const [loading2, setLoading2] = useState(null);
  const [snapped, setSnapped] = useState(seedWeight);
  const BAR_LB = 45;
  const PLATES_LB = [
  { lb:45,  label:"45",  name:"Red",    color:"#CC2200", textColor:"#fff" },
  { lb:35,  label:"35",  name:"Blue",   color:"#1a4fc4", textColor:"#fff" },
  { lb:25,  label:"25",  name:"Yellow", color:"#d4a800", textColor:"#111" },
  { lb:10,  label:"10",  name:"Green",  color:"#1e7a2e", textColor:"#fff" },
  { lb:5,   label:"5",   name:"White",  color:"#d8d8d8", textColor:"#111" },
  { lb:2.5, label:"2.5", name:"Black",  color:"#1a1a1a", textColor:"#eee" },
];
  // Use kg plates if kg unit, lbs plates if lbs unit
  const PLATES_ACTIVE = isKg ? PLATES_DATA : PLATES_LB;
  const BAR_ACTIVE = isKg ? BAR_KG : 45;
  const SNAP_INC = isKg ? 2.5 : 5;
  const calcL = (target) => {
    let rem = (target - BAR_ACTIVE) / 2;
    if (rem < 0) return null;
    const counts = [];
    for (const p of PLATES_ACTIVE) {
      const w = isKg ? p.kg : p.lb;
      const n = Math.floor(rem/w+0.001);
      counts.push(n); rem -= n*w;
    }
    return Math.abs(rem) > 0.05 ? null : counts;
  };
  const snapL = (val) => val<=BAR_ACTIVE ? BAR_ACTIVE : BAR_ACTIVE + Math.round((val-BAR_ACTIVE)/SNAP_INC)*SNAP_INC;
  ...
  const perSide = loading2 ? PLATES_ACTIVE.reduce((s,p,i)=>s+(isKg?p.kg:p.lb)*(loading2[i]||0),0) : 0;

  const PlateRow = ({counts,side,plates_data,is_kg}) => {
    if(!counts) return null;
    const PDATA = plates_data || PLATES_LB;
    let plates = [];
    PDATA.forEach((p,i)=>{ for(let j=0;j<(counts[i]||0);j++) plates.push({...p,key:`${i}-${j}`}); });
    if(side==="left") plates = [...plates].reverse();
    return (
      <div style={{display:"flex",alignItems:"center",gap:2,flexShrink:0}}>
        {plates.map(p=>{
          const pw = is_kg ? (p.kg||p.lb||10) : (p.lb||10);
          const w=Math.max(12,pw*0.55), h=52+pw*1.4;
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:G.card,borderTop:`1px solid ${G.border}`,zIndex:301,maxHeight:"88vh",overflowY:"auto",paddingBottom:32}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 20px 16px",borderBottom:`1px solid ${G.border}`}}>
          <div>
            ...
            <div style={{fontFamily:F.display,fontSize:26,color:G.text,letterSpacing:3}}>LOAD THE BAR</div>
          </div>
        ...
        <div style={{padding:"20px 20px 0"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>
            {(isKg?[60,80,100,120,140,160,180,200]:[135,185,225,275,315,365,405,455]).map(v=>(
              <button key={v} onClick={()=>setInputVal(String(v))} style={{
                background:parseFloat(snapped)===v?G.border:"#0a0a0a",color:parseFloat(snapped)===v?G.text:G.muted,
                border:`1px solid ${G.border}`,padding:"5px 11px",
          ...
          {loading2&&(
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {[["TOTAL",`${snapped} ${isKg?"kg":"lbs"}`],["BAR",`${BAR_ACTIVE} ${isKg?"kg":"lbs"}`],["PER SIDE",`${perSide} ${isKg?"kg":"lbs"}`]].map(([lbl,val])=>(
                <div key={lbl} style={{flex:1,background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"10px 8px",textAlign:"center"}}>
                  ...
                  <div style={{fontFamily:F.display,fontSize:16,color:G.text,letterSpacing:1}}>{val}</div>
                </div>
              ))}
            </div>
          )}
          {loading2&&(
            <div style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"16px 12px",marginBottom:12,overflowX:"auto"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",minWidth:"max-content",margin:"0 auto"}}>
                ...
                <PlateRow counts={loading2} side="left" plates_data={PLATES_ACTIVE} is_kg={isKg}/>
                ...
                <div style={{height:14,minWidth:60,flex:"0 1 80px",background:`linear-gradient(180deg,${G.muted},${G.border})`,borderRadius:2,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <span style={{fontSize:7,fontFamily:F.display,letterSpacing:2,color:G.text}}>BAR</span>
                </div>
                <div style={{width:8,height:60,background:G.border,borderRadius:2,flexShrink:0,margin:"0 2px"}}/>
                <PlateRow counts={loading2} side="right" plates_data={PLATES_ACTIVE} is_kg={isKg}/>
          ...
          {loading2&&(
            <div style={{background:"#0a0a0a",border:`1px solid ${G.border}`,overflow:"hidden",marginBottom:16}}>
              ...
              {loading2.map((count,i)=>count>0?(
                <div key={i} style={{display:"flex",alignItems:"center",padding:"10px 14px",gap:10,borderBottom:`1px solid ${G.border}`}}>
                  <div style={{width:32,height:32,background:PLATES_ACTIVE[i]?.color||G.border,border:"1px solid rgba(255,255,255,0.1)",borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <span style={{fontFamily:F.display,fontSize:11,color:PLATES_ACTIVE[i]?.textColor||G.text,letterSpacing:1}}>{PLATES_ACTIVE[i]?.label}</span>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:F.display,fontSize:16,letterSpacing:2,color:G.text}}>{PLATES_ACTIVE[i]?.name?.toUpperCase()} — {isKg?PLATES_ACTIVE[i]?.kg:PLATES_ACTIVE[i]?.lb} {isKg?"KG":"LB"}</div>
                  </div>
                  <div style={{display:"flex",gap:6}}>
                    {[["SIDE",count,true],["TOTAL",count*2,false]].map(([lbl,val,hi])=>(
                      <div key={lbl} style={{textAlign:"center"}}>
                        ...
                        <div style={{background:hi?goldGrad:G.card2,color:hi?G.text:G.muted,padding:"3px 10px",fontFamily:F.display,fontSize:20,border:hi?"none":`1px solid ${G.border}`}}>{val}</div>
                      </div>
                    ))}
                  </div>
                </div>
...
function SessionModal({s,rms,unit,completed,onToggle,onClose,onLog,logs,accLogs,setAccLog,onTimer}) {
  ...
  const [phase,setPhase]         = useState("overview");  // overview → checkin → session → wins
  ...
  const [inlineTimer,setInlineTimer] = useState({running:false,secs:0,total:0,round:1,rounds:6,done:false,countdown:0});
  const timerRef = useRef(null);

  const startInlineTimer = (proto) => {
    const total = proto==="E3MOM"?180:proto==="E2MOM"?120:60;
    setInlineTimer(t=>({...t,running:false,secs:total,total,round:1,done:false,countdown:0}));
  };

  useEffect(()=>{
    if (!inlineTimer.running) { clearInterval(timerRef.current); return; }
    const beep=(freq,dur,vol)=>{try{const ctx=new(window.AudioContext||window.webkitAudioContext)();const o=ctx.createOscillator(),g=ctx.createGain();o.connect(g);g.connect(ctx.destination);o.frequency.value=freq;g.gain.setValueAtTime(vol,ctx.currentTime);g.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+dur);o.start();o.stop(ctx.currentTime+dur);}catch(e){}};
    timerRef.current = setInterval(()=>{
      setInlineTimer(t=>{
        if (t.countdown>0) {
          if (t.countdown<=3) beep(660,0.08,0.4);
          if (t.countdown===1) setTimeout(()=>beep(1046,0.25,0.8),950);
          return {...t,countdown:t.countdown-1};
        }
        if (t.secs<=1) {
          beep(1046,0.2,0.7);
          if (t.round>=t.rounds) return {...t,running:false,done:true,secs:t.total};
          return {...t,round:t.round+1,secs:t.total};
        }
        if (t.secs<=4) beep(440,0.1,0.3);
  ...
  const getLiftTop = (ex) => {
    const rm=parseFloat(rms[ex.name]);
    if(!rm||isNaN(rm)) return null;
    const adj = readiness ? readiness.adj/100 : 0;
    const topPct = ex.pat?.tl==="94-102%" ? 0.98 : ex.pat?.tp||0.85;
    return r25(rm*(topPct+adj), unit);
  };

  const handleSetDone = () => {
    if (cur.type==="main" && cur.row.isTop && cur.top) onLog(s.id, cur.ex.name, {topWeight: curWt||cur.top});
    if (cur.type==="rpe" && curWt) setAccLog(s.id, cur.ex.name, cur.setIdx, curWt);
    if (cur.type==="main") setPQ(true);
    else advance();
  };

  const advance = () => {
    setPQ(false);
    // Reset inline timer for next exercise
    if (steps[stepIdx+1] && steps[stepIdx+1].ex.name !== cur?.ex?.name) {
      const nextProto = steps[stepIdx+1]?.ex?.proto || "EMOM";
      const total_s = nextProto==="E3MOM"?180:nextProto==="E2MOM"?120:60;
      setInlineTimer(t=>({...t,running:false,secs:total_s,total:total_s,round:1,done:false}));
    }
  ...
  // ── Week/theme lookup ─────────────────────────────────────────────────────
  const weekData = PROGRAM.find(w=>w.sessions.some(ss=>ss.id===s.id));

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(4,4,4,0.97)",zIndex:200,overflowY:"auto",padding:"12px 10px"}}>
      {barTarget!==null&&<BarLoaderSheet seedWeight={barTarget} userUnit={unit} onClose={()=>setBarTarget(null)}/>}
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        <div style={{background:G.card2,padding:"14px 16px",borderBottom:`1px solid ${G.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            <span style={{fontFamily:F.display,background:goldGrad,color:G.text,padding:"1px 10px",fontSize:16,letterSpacing:2}}>S{s.id}</span>
            <span style={{fontFamily:F.display,fontSize:20,letterSpacing:2,color:G.text}}>{s.day}</span>
            <span style={{fontFamily:F.display,fontSize:12,color:G.muted,letterSpacing:2,background:G.card,padding:"1px 10px",border:`1px solid ${G.border}`}}>{s.focus.toUpperCase()}</span>
        ...
        {/* Gradient accent */}
        <div style={{height:2,background:goldGrad}}/>

        {isPR&&phase!=="overview"&&<div style={{background:G.card2,padding:"9px 16px",borderBottom:`1px solid ${G.border}`}}>
          <span style={{fontFamily:F.display,fontSize:16,color:G.text,letterSpacing:3}}>🏆 PR WEEK — LEAVE EVERYTHING IN THE TANK</span>
        </div>}
        ...
        {phase==="overview"&&(
          <div style={{padding:"24px 16px"}}>
            ...
            {/* Week + theme */}
            {weekData&&(
              <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:4,marginBottom:16}}>
                WEEK {weekData.week} — {weekData.theme.toUpperCase()} · {weekData.range}
              </div>
            )}
            ...
            {mainEx.length>0&&(
              <div style={{marginBottom:20}}>
                <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:4,marginBottom:12}}>MAIN LIFTS</div>
                {mainEx.map((ex,i)=>{
                  const top = getLiftTop(ex);
                  return (
                    <div key={i} style={{borderBottom:`1px solid ${G.border}`,paddingBottom:14,marginBottom:14}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                        <div>
                          <div style={{fontFamily:F.display,fontSize:24,color:G.text,letterSpacing:0,lineHeight:1,marginBottom:4}}>{ex.name.toUpperCase()}</div>
                          <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:3}}>{ex.proto} · {ex.pat?.reps[2]} sets @ {ex.pat?.tl}</div>
                        </div>
                        {top&&<div style={{textAlign:"right"}}>
                          <div style={{fontFamily:F.display,fontSize:28,color:G.text,letterSpacing:1,lineHeight:1}}>{top}</div>
                          <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:2}}>{unit} TOP</div>
                        </div>}
                      </div>
                      {/* Set rows */}
                      {top&&(()=>{
                        const W=[r25(top*0.70,unit),r25(top*0.85,unit),top,r25(top*0.88,unit),r25(top*0.82,unit)];
                        const labels=["Build 1","Build 2","Top Set","Back-off 1","Back-off 2"];
                        const reps=ex.pat?.reps||[3,3,3,6,9];
                        return (
                          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                            {W.map((w,j)=>(
                              <div key={j} style={{
                                flex:"1 1 auto",
                                background:j===2?G.card2:G.card,
                                border:`1px solid ${j===2?"#333":G.border}`,
                                padding:"8px 10px",textAlign:"center",minWidth:52
                              }}>
                                <div style={{fontFamily:F.display,fontSize:8,color:G.muted,letterSpacing:1,marginBottom:3}}>{labels[j].toUpperCase()}</div>
                                <div style={{fontFamily:F.display,fontSize:16,color:j===2?G.text:G.muted,letterSpacing:1}}>{w}</div>
                                <div style={{fontFamily:F.body,fontSize:9,color:G.muted,marginTop:2}}>{reps[j]} rep{reps[j]!==1?"s":""}</div>
                              </div>
                            ))}
                          </div>
                        );
                      })()}
                      {!top&&<div style={{fontFamily:F.body,fontSize:11,color:G.muted}}>Set your 1RM in Profile to see weights</div>}
                    </div>
                  );
                })}
              </div>
            ...
            {accEx.length>0&&(
              <div style={{marginBottom:24}}>
                <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:4,marginBottom:12}}>ACCESSORY WORK</div>
                {accEx.map((ex,i)=>{
                  const isSuper = ex.type==="superset";
                  const name = isSuper ? `${ex.exA.name} + ${ex.exB.name}` : ex.name;
                  const sets = isSuper ? ex.exA.sets : ex.sets;
                  const proto = isSuper ? ex.exA.proto : ex.proto;
                  return (
                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                      borderBottom:`1px solid ${G.border}`,paddingBottom:10,marginBottom:10}}>
                      <div>
                        {isSuper&&<div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:3}}>SUPERSET</div>}
                        <div style={{fontFamily:F.display,fontSize:15,color:G.text,letterSpacing:1,lineHeight:1.2}}>{name}</div>
                        <div style={{fontFamily:F.body,fontSize:10,color:G.muted,marginTop:3}}>{proto}</div>
                      </div>
                      <div style={{textAlign:"right",flexShrink:0,marginLeft:12}}>
                        <div style={{fontFamily:F.display,fontSize:20,color:G.text,letterSpacing:1}}>{sets.length}</div>
                        <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:2}}>SETS</div>
                        {sets[0]?.rpe&&<div style={{fontFamily:F.body,fontSize:9,color:G.muted,marginTop:2}}>RPE {sets[0].rpe}–{sets[sets.length-1].rpe||sets[sets.length-2]?.rpe}</div>}
                      </div>
                    </div>
                  );
                })}
            ...
            {/* Total sets count */}
            <div style={{display:"flex",gap:12,marginBottom:24,padding:"12px 16px",background:G.card2,border:`1px solid ${G.border}`}}>
              {[
                ["TOTAL SETS", total],
                ["MAIN", steps.filter(s=>s.type==="main").length],
                ["ACCESSORY", steps.filter(s=>s.type==="rpe").length],
              ].map(([lbl,val])=>(
                <div key={lbl} style={{flex:1,textAlign:"center"}}>
                  <div style={{fontFamily:F.display,fontSize:22,color:G.text,letterSpacing:1}}>{val}</div>
                  <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:2}}>{lbl}</div>
                </div>
              ))}
            </div>
            ...
            {isDone?(
              <button onClick={onClose}
                style={{width:"100%",padding:16,background:"transparent",border:`1px solid ${G.border}`,
                  fontFamily:F.display,fontSize:20,letterSpacing:4,color:G.muted,cursor:"pointer"}}>
                ✓ ALREADY COMPLETE
              </button>
            ):(
              <button onClick={()=>setPhase("checkin")}
                style={{width:"100%",padding:18,background:goldGrad,border:"none",
                  fontFamily:F.display,fontSize:28,letterSpacing:0,color:G.text,cursor:"pointer"}}>
                START SESSION →
              </button>
            )}
          </div>
        ...
        {/* ══ CHECK-IN ══ */}
        {phase==="checkin"&&<ReadinessGate onComplete={data=>{
          setReadiness(data);
          // Init timer for first exercise
          if (steps[0]) {
            const proto=steps[0].ex.proto||"EMOM";
            const ts=proto==="E3MOM"?180:proto==="E2MOM"?120:60;
            setInlineTimer({running:false,secs:ts,total:ts,round:1,rounds:6,done:false,countdown:0});
          }
          setPhase("session");
        }}/>}
          ...
          <div>
            {readiness&&readiness.adj!==0&&(
              <div style={{padding:"8px 16px",background:G.card2,borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontFamily:F.display,fontSize:14,color:G.muted,letterSpacing:2}}>{readiness.adj}%</span>
              </div>
            )}
            {/* Progress */}
            <div style={{padding:"10px 16px 8px",borderBottom:`1px solid ${G.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                ...
                <span style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:2}}>{stepIdx+1}/{total}</span>
              </div>
              <div style={{height:2,background:"#1a1a1a"}}>
                <div style={{height:2,background:goldGrad,width:`${progress*100}%`,transition:"width 0.4s"}}/>
              </div>
            </div>
            ...
            {cur.type==="main"&&(()=>{
              const T=inlineTimer;
              const mm=String(Math.floor(T.secs/60)).padStart(2,"0");
              const ss=String(T.secs%60).padStart(2,"0");
              const pct=T.total>0?(T.total-T.secs)/T.total:0;
              const timeColor=T.secs>T.total*0.6?G.text:T.secs>T.total*0.3?G.yellow:G.red;
              ...
              <div style={{padding:"16px"}}>
                ...
                <div style={{textAlign:"center",marginBottom:12}}>
                  <div style={{fontFamily:F.display,fontSize:11,color:cur.row.isTop?G.text:G.muted,letterSpacing:4,marginBottom:4}}>
                    {cur.row.ro.toUpperCase()} · {cur.row.lb.toUpperCase()}
                  </div>
                  <div style={{fontFamily:F.display,fontSize:36,color:G.text,letterSpacing:3,lineHeight:1}}>
                    {cur.ex.name.toUpperCase()}
                  </div>
                </div>

                {/* Weight + Timer side by side */}
                <div style={{display:"flex",gap:0,marginBottom:16,border:`1px solid ${G.border}`,background:G.card2}}>

                  {/* Weight */}
                  <div style={{flex:1,padding:"20px 16px",textAlign:"center",borderRight:`1px solid ${G.border}`}}>
                    <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:3,marginBottom:6}}>WEIGHT</div>
                    <div style={{fontFamily:F.display,fontSize:56,color:G.text,lineHeight:1,marginBottom:2}}>
                      {cur.row.w||"--"}
                    </div>
                    <div style={{fontFamily:F.display,fontSize:13,color:G.muted,letterSpacing:2}}>{unit.toUpperCase()}</div>
                    <div style={{fontFamily:F.body,fontSize:11,color:G.muted,marginTop:6}}>
                      {cur.row.reps} {cur.row.reps===1?"rep":"reps"} · {cur.row.tag}
                    </div>
                    {cur.row.w&&(
                      <button onClick={()=>setBarTarget(cur.row.w)}
                        style={{marginTop:10,padding:"5px 12px",background:"transparent",border:`1px solid ${G.border}`,
                          fontFamily:F.display,fontSize:10,letterSpacing:2,color:G.muted,cursor:"pointer"}}>
                        🏋 PLATES
                      </button>
                    )}
                  </div>
                  ...
                  {/* Inline Timer */}
                  <div style={{flex:1,padding:"20px 16px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:3,marginBottom:6}}>{cur.ex.proto}</div>
                    ...
                    <div style={{position:"relative",width:64,height:64,marginBottom:8}}>
                      <svg width={64} height={64}>
                        ...
                        <circle cx={32} cy={32} r={R} fill="none" stroke={timeColor} strokeWidth={5}
                          strokeDasharray={C} strokeDashoffset={C*(1-pct)} strokeLinecap="round"
                          style={{transform:"rotate(-90deg)",transformOrigin:"32px 32px",transition:"stroke-dashoffset 0.9s linear"}}/>
                      </svg>
                      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <div style={{fontFamily:F.display,fontSize:T.countdown>0?28:T.secs>0?18:14,color:T.countdown>0?G.red:timeColor,lineHeight:1,letterSpacing:1}}>
                          {T.countdown>0?T.countdown:T.secs>0?`${mm}:${ss}`:"GO"}
                        </div>
                      </div>
                    </div>

                    <div style={{fontFamily:F.body,fontSize:10,color:G.muted,marginBottom:10}}>
                      {T.done?"DONE":T.running?`R ${T.round}/${T.rounds}`:`R ${T.round}/${T.rounds}`}
                    </div>

                    {/* Round counter */}
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                      <button onClick={()=>setInlineTimer(t=>({...t,rounds:Math.max(1,t.rounds-1)}))}
                        style={{width:22,height:22,background:"#1a1a1a",border:`1px solid ${G.border}`,color:G.text,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                      <span style={{fontFamily:F.display,fontSize:16,color:G.text,minWidth:20,textAlign:"center"}}>{T.rounds}</span>
                      <button onClick={()=>setInlineTimer(t=>({...t,rounds:Math.min(30,t.rounds+1)}))}
                        style={{width:22,height:22,background:"#1a1a1a",border:`1px solid ${G.border}`,color:G.text,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                    </div>

                    {/* Start/Pause/Reset */}
                    <div style={{display:"flex",gap:6,width:"100%"}}>
                      <button onClick={()=>setInlineTimer(t=>({...t,running:false,secs:t.total,round:1,done:false,countdown:0}))}
                        style={{flex:1,padding:"7px 0",background:"#1a1a1a",border:`1px solid ${G.border}`,
                          fontFamily:F.display,fontSize:9,letterSpacing:1,color:G.muted,cursor:"pointer"}}>
                        RST
                      </button>
                      <button onClick={()=>{
                        if (T.done) { setInlineTimer(t=>({...t,running:false,secs:t.total,round:1,done:false,countdown:0})); return; }
                        if (!T.running) {
                          if (T.secs===T.total && T.countdown===0) {
                            setInlineTimer(t=>({...t,running:true,countdown:10}));
                          } else {
                            setInlineTimer(t=>({...t,running:!t.running}));
                          }
                        } else {
                          setInlineTimer(t=>({...t,running:false}));
                        }
                      }}
                        style={{flex:2,padding:"7px 0",background:T.done?goldGrad:T.running?"#1a0808":"#1a1a1a",
                          border:"none",fontFamily:F.display,fontSize:11,letterSpacing:2,
                          color:T.done?G.text:T.running?G.red:G.text,cursor:"pointer"}}>
                        {T.done?"↺":T.running?"⏸":"▶ GO"}
                      </button>
                    </div>
                  </div>
                ...
                <div style={{marginBottom:20}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",background:G.card2,border:`1px solid ${G.border}`}}>
                ...
                <div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:20}}>
                  {["i","ii","iii","iv","v"].map((ro,i)=>{
                    const firstStepOfLift = steps.findIndex(st=>st.ex.name===cur.ex.name&&st.type==="main");
                    const relIdx = stepIdx - firstStepOfLift;
                    return (
                      <div key={ro} style={{width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",
                        ...
                        background:i<relIdx?"#222":i===relIdx?goldGrad:G.card2,
                        border:`1px solid ${i===relIdx?"transparent":G.border}`,
                        color:i<relIdx?G.muted:i===relIdx?G.text:G.muted}}>
                        {i<relIdx?"✓":ro}
                      </div>
                    );
                  })}
                ...
                {pendingQuality?(
                  <div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5,marginBottom:10}}>
                      {QUALITY.map(q=>(
                        <button key={q.v} onClick={()=>{setQR(p=>({...p,[cur.ex.name]:[...(p[cur.ex.name]||[]),q.v]}));advance();}}
                          style={{padding:"12px 0",background:G.card2,border:`1px solid ${G.border}`,cursor:"pointer",
                            display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                          <span style={{fontFamily:F.display,fontSize:22,color:G.text}}>{q.v}</span>
                          <span style={{fontFamily:F.body,fontSize:7,color:G.muted,textTransform:"uppercase",letterSpacing:1,textAlign:"center"}}>{q.label}</span>
                        </button>
                      ))}
                    </div>
                  ...
                  <button onClick={handleSetDone}
                    style={{width:"100%",padding:16,background:goldGrad,border:"none",
                      fontFamily:F.display,fontSize:22,letterSpacing:4,color:G.text,cursor:"pointer"}}>
            ...
            {cur.type==="rpe"&&(
              <div style={{padding:"24px 16px"}}>
                ...
                <div style={{textAlign:"center",marginBottom:20}}>
                  <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:4,marginBottom:4}}>SET {cur.setIdx+1}</div>
                  <div style={{fontFamily:F.display,fontSize:36,color:G.text,letterSpacing:3,lineHeight:1,marginBottom:8}}>
                    {cur.ex.name.toUpperCase()}
                  </div>
                  <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
                    ...
                    {cur.set?.rpe!=null&&(
                      <span style={{fontFamily:F.display,fontSize:20,letterSpacing:2,color:G.muted,
                        background:G.card2,padding:"2px 12px",border:`1px solid ${G.border}`}}>
                        RPE {cur.set.rpe}
                      </span>
                    )}
                ...
                {/* Compact inline timer for accessories */}
                {(()=>{
                  const T=inlineTimer;
                  const mm=String(Math.floor(T.secs/60)).padStart(2,"0");
                  const ss=String(T.secs%60).padStart(2,"0");
                  ...
                  return (
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,padding:"10px 14px",
                      ...
                      <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:3,flex:1}}>{cur.ex.proto}</div>
                      <div style={{fontFamily:F.display,fontSize:22,color:T.countdown>0?G.red:timeColor,letterSpacing:1,minWidth:52,textAlign:"center"}}>
                        {T.countdown>0?T.countdown:T.secs>0?`${mm}:${ss}`:"--:--"}
                      </div>
                      <div style={{fontFamily:F.body,fontSize:9,color:G.muted,minWidth:36,textAlign:"center"}}>
                        {T.done?"DONE":`R${T.round}/${T.rounds}`}
                      </div>
                      <button onClick={()=>setInlineTimer(t=>({...t,running:false,secs:t.total,round:1,done:false,countdown:0}))}
                        style={{background:"#1a1a1a",border:`1px solid ${G.border}`,color:G.muted,
                          padding:"5px 8px",fontFamily:F.display,fontSize:9,letterSpacing:1,cursor:"pointer"}}>RST</button>
                      <button onClick={()=>{
                        if(T.done){setInlineTimer(t=>({...t,running:false,secs:t.total,round:1,done:false}));return;}
                        if(!T.running){
                          if(T.secs===T.total&&T.countdown===0){setInlineTimer(t=>({...t,running:true,countdown:10}));}
                          else{setInlineTimer(t=>({...t,running:true}));}
                        }else{setInlineTimer(t=>({...t,running:false}));}
                      }} style={{background:T.running?"#1a0808":"#1a1a1a",border:"none",
                        color:T.running?G.red:G.text,padding:"5px 12px",fontFamily:F.display,
        ...
        {phase==="complete_acc"&&(
          <div style={{padding:"28px 16px",textAlign:"center"}}>
            ...
            <div style={{fontFamily:F.display,fontSize:40,color:G.text,letterSpacing:0,lineHeight:1,marginBottom:24}}>SESSION COMPLETE</div>
            <button onClick={isDone?onClose:handleComplete}
              style={{width:"100%",padding:16,background:goldGrad,border:"none",
                fontFamily:F.display,fontSize:20,letterSpacing:3,color:G.text,cursor:"pointer"}}>
...
function EMOMTimer({proto,exName,onClose}) {
  ...
  const timeColor=secs>totalSecs*0.6?G.text:secs>totalSecs*0.3?G.yellow:G.red;
  const R_sm=18,C_sm=2*Math.PI*R_sm;

  return (
    <div style={{position:"fixed",bottom:80,right:12,zIndex:400,background:G.card,
      border:`1px solid ${flash?"#555":G.border}`,borderTop:`2px solid ${flash?G.teal:G.purple}`,borderRadius:12,minWidth:200,maxWidth:260,
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:"#0a0a0a",borderRadius:"12px 12px 0 0",borderBottom:`1px solid ${G.border}`}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:F.display,fontSize:13,color:G.muted,letterSpacing:3}}>{proto}</div>
      ...
      {minimized&&(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
          ...
          <button onClick={()=>done?reset():setRunning(r=>!r)}
            style={{background:done?goldGrad:running?"#1a0808":"#1a1a1a",border:"none",borderRadius:6,color:G.text,padding:"6px 10px",fontSize:14,cursor:"pointer"}}>
            {done?"↺":running?"⏸":"▶"}
          </button>
        </div>
      )}
      {!minimized&&(
        <div style={{padding:"12px 14px"}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
            ...
            <div style={{flex:1}}>
              {done?<div style={{fontSize:13,color:G.text,fontWeight:800}}>🏆 Done!</div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>done?reset():setRunning(r=>!r)}
              style={{flex:3,padding:"9px",background:done?goldGrad:running?"#1a0808":"#1a1a1a",
                border:"none",borderRadius:6,color:done?G.text:running?G.red:G.text,
                fontWeight:800,fontSize:11,letterSpacing:2,cursor:"pointer"}}>
              {done?"START OVER":running?"⏸ PAUSE":"▶ START"}
            </button>
          </div>
          {flash&&<div style={{marginTop:8,textAlign:"center",fontSize:10,color:G.muted,fontWeight:800,letterSpacing:2}}>NEXT ROUND ▶</div>}
        </div>
      )}
    </div>
  );
...
function CoachDashboard({profile, onSignOut}) {
  ...
  const cardStyle = {background:"#0f0f0f",border:"1px solid #2a2a2a",borderRadius:10};
  ...
  if (selected) {
    ...
    return (
      <div style={{minHeight:"100vh",background:"#000000",color:"#F0F0F0",fontFamily:"system-ui,sans-serif",padding:16}}>
        ...
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 0",marginBottom:16}}>
            <button onClick={()=>{setSelected(null);setProgress(null);}}
              style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:6,color:"#666666",padding:"6px 14px",fontSize:11,cursor:"pointer"}}>← BACK</button>
            <div>
              <div style={{fontWeight:900,fontSize:18,color:"#F0F0F0"}}>{selected.name}</div>
              <div style={{fontSize:10,color:"#666666",letterSpacing:2}}>ATHLETE PROFILE</div>
            </div>
          </div>

          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.text,fontWeight:800,letterSpacing:3,marginBottom:12}}>PROGRAM PROGRESS</div>
            ...
            <div style={{fontSize:12,color:G.muted,fontWeight:700}}>{completed.size}/16 sessions completed</div>
          </div>

          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.text,fontWeight:800,letterSpacing:3,marginBottom:12}}>1RM ({unit})</div>
            {["Hang Clean & Press","Squat","Bench","Deadlift"].map(l=>(
              <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #1e1e1e"}}>
                ...
                <span style={{fontSize:13,fontWeight:800,color:"#F0F0F0"}}>{rms[l]||"—"} {rms[l]?unit:""}</span>
              </div>
            ))}
          </div>

          <div style={{...cardStyle,padding:16,marginBottom:14}}>
            <div style={{fontSize:10,color:G.text,fontWeight:800,letterSpacing:3,marginBottom:12}}>SESSION LOG</div>
                ...
                <div key={s.id} style={{padding:"10px 0",borderBottom:"1px solid #1e1e1e"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:done&&sl?6:0}}>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:10,background:done?"#1a1a1a":"#1a1a1a",color:done?G.text:"#2a2a2a",padding:"2px 8px",borderRadius:3,fontWeight:800}}>S{s.id}</span>
                      <span style={{fontSize:11,color:done?"#F0F0F0":"#2a2a2a"}}>{s.day} — {s.focus}</span>
                    </div>
                  ...
                  {done&&sl&&Object.entries(sl).map(([ex,entries])=>(
                    <div key={ex} style={{paddingLeft:12,marginTop:4}}>
                      {entries.map((e,i)=>(
                        <div key={i} style={{fontSize:10,color:"#666666"}}>
                          {ex}: <span style={{color:G.text,fontWeight:700}}>{e.topWeight} {unit}</span>
                        </div>
                      ))}
                    </div>
                  ))}
  ...
  return (
    <div style={{minHeight:"100vh",background:"#000000",color:"#F0F0F0",fontFamily:"system-ui,sans-serif"}}>
      <div style={{height:3,background:goldGrad}}/>
      <header style={{background:"rgba(8,8,8,0.97)",borderBottom:"1px solid #2a2a2a",padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <Logo size="sm"/>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:9,color:"#666666",letterSpacing:1}}>COACH</div>
          <div style={{fontSize:13,fontWeight:800,color:G.text}}>{profile.name}</div>
        </div>
      </header>
      <main style={{maxWidth:680,margin:"0 auto",padding:"16px 14px"}}>
        <div style={{fontSize:10,color:G.muted,fontWeight:800,letterSpacing:3,marginBottom:14}}>
          ATHLETES ({athletes.length})
        </div>
        {loading&&<div style={{color:"#666666",fontSize:12,textAlign:"center",padding:40}}>Loading athletes...</div>}
        {!loading&&athletes.length===0&&(
          <div style={{...cardStyle,padding:24,textAlign:"center"}}>
            <div style={{fontSize:13,color:"#666666"}}>No athletes registered yet.</div>
        ...
        {athletes.map(a=>{
          return (
            <button key={a.id} onClick={()=>selectAthlete(a)}
              ...
              <div>
                <div style={{fontFamily:"'Bebas Neue', sans-serif",fontSize:20,letterSpacing:2,color:"#F0F0F0",marginBottom:2}}>{a.name}</div>
                <div style={{fontSize:10,color:"#666666",letterSpacing:1}}>
        ...
        <button onClick={onSignOut}
          style={{width:"100%",padding:12,background:"transparent",border:"1px solid #2a2a2a",borderRadius:8,color:"#666666",fontSize:11,cursor:"pointer",marginTop:20,letterSpacing:2}}>
          SIGN OUT
        </button>
      </main>
    </div>
...
const PLATES_DATA = [
  ...
  { kg:15,   label:"15",   name:"Yellow", color:"#d4a800", textColor:"#111" },
  { kg:10,   label:"10",   name:"Green",  color:"#1e7a2e", textColor:"#fff" },
  { kg:5,    label:"5",    name:"White",  color:"#d8d8d8", textColor:"#111" },
  { kg:2.5,  label:"2.5",  name:"Black",  color:"#1a1a1a", textColor:"#eee" },
  { kg:1.25, label:"1.25", name:"Silver", color:"#aaaaaa", textColor:"#111" },
  { kg:0.5,  label:"0.5",  name:"Silver", color:"#cccccc", textColor:"#111" },
];
...
function BarLoader() {
  ...
  const PlateBar = ({counts, side}) => {
    if (!counts) return null;
    let plates = [];
    PLATES_DATA.forEach((p,i)=>{ for(let j=0;j<counts[i];j++) plates.push({...p,key:`${i}-${j}`}); });
    // Left side: sleeve(left) → lightest...heaviest → collar(right) → reverse so heaviest near collar
    if(side==="left") plates = [...plates].reverse();
    // Right side: collar(left) → heaviest...lightest → sleeve(right) → natural order
    return (
      <div style={{display:"flex",alignItems:"center",gap:2,flexShrink:0}}>
        {plates.map(p=>{
      ...
      <div style={{...cs,padding:16}}>
        ...
        {loading2 && parseFloat(inputVal)!==parseFloat(dispSnapped) && (
          <div style={{marginTop:8,fontSize:11,color:G.muted,textAlign:"center",fontFamily:F.body}}>
            Snapped to <span style={{color:G.text,fontWeight:700}}>{dispSnapped} {blUnit}</span>
          </div>
        )}
        {!loading2 && inputVal && (
          <div style={{marginTop:8,fontSize:11,color:G.muted,textAlign:"center",fontFamily:F.body}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          {[["TOTAL",`${snapped.toFixed(1)} kg`,`${(snapped*2.20462).toFixed(1)} lbs`],
            ...
            <div key={lbl} style={{background:G.card,border:`1px solid ${G.border}`,padding:"12px 8px",textAlign:"center"}}>
              ...
              <div style={{fontFamily:F.display,fontSize:18,color:G.text,letterSpacing:1}}>{val}</div>
      ...
      {loading2 && (
        <div style={{...cs,padding:20,overflowX:"auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:0,minWidth:"max-content",margin:"0 auto"}}>
            ...
            <PlateBar counts={loading2} side="left"/>
            ...
            <div style={{width:10,height:72,background:G.border,borderRadius:2,flexShrink:0,margin:"0 2px"}}/>
            <PlateBar counts={loading2} side="right"/>
      ...
      {loading2 && (
        <div style={cs}>
          ...
          {PLATES_DATA.map((p,i)=>loading2[i]>0&&(
            <div key={p.name} style={{display:"flex",alignItems:"center",padding:"12px 16px",borderBottom:`1px solid ${G.border}`,gap:12}}>
              <div style={{width:36,height:36,background:p.color||G.border,border:"1px solid rgba(255,255,255,0.1)",borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                <span style={{fontFamily:F.display,fontSize:11,color:p.textColor||G.text,letterSpacing:1}}>{p.label}</span>
              </div>
              <div style={{flex:1}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {[["PER SIDE",loading2[i],true],["TOTAL",loading2[i]*2,false]].map(([lbl,val,hi])=>(
                  <div key={lbl} style={{textAlign:"center"}}>
                    ...
                    <div style={{background:hi?goldGrad:G.card2,color:hi?G.text:G.muted,padding:"4px 12px",fontFamily:F.display,fontSize:22,letterSpacing:1,border:hi?"none":`1px solid ${G.border}`}}>
                      {val}
                    </div>
                  </div>
                ))}
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {(blUnit==="lbs"?[135,185,225,275,315,365,405,455]:[60,80,100,120,140,160,180,200]).map(v=>(
            <button key={v} onClick={()=>setInputVal(String(v))}
              style={{background:parseFloat(dispSnapped)===v?goldGrad:G.card2,color:parseFloat(dispSnapped)===v?G.text:G.muted,
...
export default function App() {
  ...
  useEffect(()=>{
    ST.restoreSession().then(d=>{
      if (d) handleLogin(d);
      setLoading(false);
    });
    // Listen for auth changes
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_OUT') { setProfile(null); saveRef.current=false; }
    });
  ...
  const handleSignOut = async () => {
    await ST.signOut();
    setProfile(null); setRms({}); setUnit("lbs"); setCompleted(new Set());
    setLogs({}); setAccLogs({}); setMevSel({}); saveRef.current=false;
  };

  if (loading) return (
    <div style={{minHeight:"100vh",background:G.black,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontFamily:F.display,fontSize:48,color:G.text,letterSpacing:3}}>4:16</div>
        <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:4,marginTop:8}}>LOADING...</div>
      </div>
    </div>
  );

  if (!profile) return <Onboarding onComplete={handleLogin}/>;
  if (profile.role==='coach') return <CoachDashboard profile={profile} onSignOut={handleSignOut}/>;
  ...
  return (
    <div style={{minHeight:"100vh",background:G.black,color:G.text,fontFamily:F.body,paddingBottom:72}}>
      <div style={{height:2,background:goldGrad,position:"sticky",top:0,zIndex:51}}/>
      <header style={{background:"rgba(8,8,8,0.98)",borderBottom:`1px solid ${G.border}`,padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:3,zIndex:50}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontFamily:F.display,fontSize:18,color:G.text,letterSpacing:2,lineHeight:1}}>{profile.name}</div>
        </div>
      </header>
          ...
          <div>
            <div style={{...cardStyle,padding:16,marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                ...
                <span style={{fontSize:12,color:G.text,fontWeight:800}}>{completed.size}/16 SESSIONS</span>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                {PROGRAM.map(w=>{const wd=w.sessions.filter(s=>completed.has(s.id)).length;return(
                  <div key={w.week} style={{textAlign:"center",background:"#0a0a0a",borderRadius:8,padding:"10px 4px"}}>
                    <div style={{fontSize:18,fontWeight:900,color:wd===4?G.text:wd>0?G.muted:"#2a2a2a"}}>{wd}/4</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
              {[["Done",completed.size],["Left",16-completed.size],["Week",Math.min(4,Math.ceil(completed.size/4)+1)]].map(([l,v])=>(
                <div key={l} style={{...cardStyle,padding:"14px 8px",textAlign:"center"}}>
                  <div style={{fontFamily:F.display,fontSize:42,color:G.text,lineHeight:1}}>{v}</div>
            ...
            {nextSession&&(
              <div style={{marginBottom:18}}>
                <div style={{fontSize:10,color:G.muted,fontWeight:800,letterSpacing:3,marginBottom:8}}>NEXT SESSION</div>
                <div onClick={()=>setModal(nextSession)}
                  style={{...cardStyle,padding:"14px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",border:`1px solid ${G.border}`}}>
                  <div>
                    <div style={{fontWeight:800,fontSize:15,color:G.text}}>Session {nextSession.id} -- {nextSession.day}</div>
                    <div style={{fontSize:11,color:G.muted,marginTop:4,letterSpacing:1}}>{nextSession.focus}</div>
                    <div style={{display:"flex",gap:5,marginTop:8,flexWrap:"wrap"}}>
                      {flatEx(nextSession.exercises).map((e,i)=>(
                        <span key={i} style={{fontSize:10,color:G.muted,background:G.card,borderRadius:3,padding:"2px 7px"}}>{e.name}</span>
                      ))}
                    </div>
                  </div>
                  <span style={{fontSize:22,color:G.muted,flexShrink:0,marginLeft:10}}>▶</span>
                </div>
              </div>
            )}
            {completed.size===16&&(
              <div style={{...cardStyle,padding:24,textAlign:"center",border:`1px solid ${G.border}`,marginBottom:18}}>
                <div style={{fontSize:40,marginBottom:8}}>🏆</div>
                <div style={{fontSize:18,fontWeight:900,color:G.text,letterSpacing:2}}>PROGRAM COMPLETE</div>
                <div style={{fontSize:11,color:G.muted,marginTop:6}}>You've peaked. Go hit those PRs.</div>
              </div>
            )}
            <div>
              <div style={{fontSize:10,color:G.muted,fontWeight:800,letterSpacing:3,marginBottom:8}}>1RM BOARD</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
                {LIFTS.map(l=>(
                  <div key={l} style={{...card2Style,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <span style={{fontSize:14,fontWeight:800,color:rms[l]?G.text:"#2a2a2a"}}>{rms[l]||"--"} <span style={{fontSize:9,color:G.muted}}>{rms[l]?unit:""}</span></span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              {PROGRAM.map(w=>{const wd=w.sessions.filter(s=>completed.has(s.id)).length,act=activeWeek===w.week;return(
                <button key={w.week} onClick={()=>setActiveWeek(w.week)}
                  style={{flex:1,padding:"10px 4px",borderRadius:8,border:`1px solid ${G.border}`,
                    background:act?G.card2:"#0a0a0a",color:act?G.text:G.muted,cursor:"pointer"}}>
                  <div style={{fontWeight:900,fontSize:13,letterSpacing:1}}>WK {w.week}</div>
                  <div style={{fontSize:10,color:act?G.muted:"#2a2a2a",marginTop:2}}>{wd}/4</div>
                </button>
              );})}
            </div>
            <div style={{padding:"8px 14px",background:G.card,borderLeft:`2px solid ${G.text}`,borderRadius:"0 4px 4px 0",marginBottom:14,display:"flex",gap:12,alignItems:"center"}}>
              <span style={{fontSize:11,color:G.text,fontWeight:800,letterSpacing:2}}>WEEK {activeWeek} -- {weekData.theme.toUpperCase()}</span>
              <span style={{fontSize:11,color:G.muted}}>{weekData.range}</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {weekData.sessions.map(s=>{
                const isDone=completed.has(s.id),fc=G.muted;
                const mainEx=s.exercises.filter(e=>e.type==="main"),accEx=s.exercises.filter(e=>e.type!=="main");
                return(
                  <div key={s.id} style={{...cardStyle,padding:"13px 14px",border:`1px solid ${G.border}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
                          <span style={{background:goldGrad,color:G.text,borderRadius:3,padding:"2px 7px",fontWeight:900,fontSize:10}}>S{s.id}</span>
                          ...
                          {isDone&&<span style={{fontSize:10,color:G.muted,fontWeight:700}}>✓ DONE</span>}
                        </div>
                        {mainEx.length>0&&<div style={{marginBottom:8}}>
                          <div style={{fontSize:8,color:G.muted,letterSpacing:2,fontWeight:700,marginBottom:5}}>MAIN LIFTS</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {mainEx.map((e,i)=><span key={i} style={{fontSize:11,color:G.muted,background:G.card,borderRadius:3,padding:"2px 8px"}}>{e.name}</span>)}
                          </div>
                        </div>}
                        {accEx.length>0&&<div>
                          <div style={{fontSize:8,color:G.muted,letterSpacing:2,fontWeight:700,marginBottom:5}}>ACCESSORY</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {accEx.map((e,i)=>e.type==="superset"
                              ?<span key={i} style={{fontSize:11,color:G.muted,background:G.card,borderRadius:3,padding:"2px 8px"}}>{e.exA.name}+{e.exB.name}</span>
                              :<span key={i} style={{fontSize:11,color:G.muted,background:G.card,borderRadius:3,padding:"2px 8px"}}>{e.name}</span>
                            )}
                          </div>
                        </div>}
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
                        <button onClick={()=>setModal(s)}
                          style={{background:"#111",border:`1px solid ${G.border}`,borderRadius:6,color:G.text,padding:"7px 12px",fontSize:10,fontWeight:700,letterSpacing:1,cursor:"pointer"}}>
                          VIEW
                        </button>
                        <button onClick={()=>toggleDone(s.id)}
                          style={{background:isDone?"#0a0a0a":"#0a0a0a",border:`1px solid ${G.border}`,
                            borderRadius:6,color:isDone?G.text:G.muted,padding:"7px 12px",fontSize:10,fontWeight:700,letterSpacing:1,cursor:"pointer"}}>
                          {isDone?"✓":"DONE?"}
                        </button>
                      </div>
                    </div>
            ...
            {completed.size===0?(
              ...
              return(
                <div key={w.week} style={{marginBottom:20}}>
                  <div style={{fontSize:10,color:G.muted,fontWeight:800,letterSpacing:3,marginBottom:10}}>
                    WEEK {w.week} -- {w.theme.toUpperCase()}
                  </div>
                  {ws.map(s=>{
                    const fc=G.muted,sl=logs[s.id],al=accLogs[s.id];
                    return(
                      <div key={s.id} style={{background:G.card,borderRadius:8,border:`1px solid ${G.border}`,marginBottom:8}}>
                          ...
                          <div style={{padding:"8px 14px",borderTop:`1px solid ${G.border}`,background:"#0a0a0a"}}>
                            {sl&&Object.entries(sl).map(([en,entries])=>(
                              <div key={en} style={{marginBottom:6}}>
                                <div style={{fontSize:10,color:G.text,fontWeight:700,marginBottom:3}}>{en}</div>
                            ...
                            {al&&Object.entries(al).map(([en,sets])=>{
                              ...
                              return <div key={en} style={{marginBottom:6}}><div style={{fontSize:10,color:G.text,fontWeight:700,marginBottom:3}}>{en}</div><div style={{fontSize:11,color:G.muted}}>{weights.join(", ")} {unit}</div></div>;
                            })}
                          </div>
                        ):null}
                      </div>
        ...
        {tab==="about"&&(
          <div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.text}>PROGRAM OVERVIEW</SH>
              <div style={{fontSize:13,color:G.text,lineHeight:1.8}}>
                ...
                {[
                  ...
                  <div key={title} style={{marginBottom:12,padding:"10px 12px",background:G.card2,borderRadius:8,border:`1px solid ${G.border}`}}>
                    <div style={{fontWeight:800,color:G.text,fontSize:12,marginBottom:4}}>{title}</div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.text}>HOW TO USE THIS APP</SH>
              {[
                ...
                <div key={i} style={{display:"flex",gap:12,marginBottom:12,alignItems:"flex-start"}}>
                  <div style={{minWidth:22,height:22,borderRadius:"50%",background:goldGrad,color:G.text,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:900,marginTop:1}}>{i+1}</div>
                  <div>
            ...
            <div style={{...cardStyle,padding:"16px",marginBottom:14}}>
              <SH color={G.text}>SET NOTATION</SH>
              {[
                ...
                <div key={term} style={{display:"flex",gap:10,marginBottom:10,padding:"8px 10px",background:G.card2,borderRadius:6,border:`1px solid ${G.border}`}}>
                  <span style={{fontWeight:800,color:G.text,fontSize:11,minWidth:80}}>{term}</span>
            ...
            <div style={{...cardStyle,padding:"20px 16px",marginBottom:14,textAlign:"center"}}>
              <SH color={G.text}>SUPPORT THE PROJECT</SH>
        ...
        {tab==="profile"&&(
          <div>
            <div style={{...cardStyle,padding:"20px 16px",marginBottom:14,textAlign:"center"}}>
              ...
              <div style={{fontSize:11,color:G.muted,marginTop:4}}>{profile.email||""}</div>
              <div style={{fontSize:9,color:G.muted,letterSpacing:3,marginTop:4}}>PROJECT 4:16 ATHLETE</div>
              <div style={{marginTop:10,fontSize:11,color:G.muted,fontWeight:700}}>{completed.size}/16 sessions completed</div>
            </div>
            <div style={{...cardStyle,padding:"14px 16px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",background:"#0a0a0a",borderRadius:6,overflow:"hidden"}}>
                {["lbs","kg"].map(u=>(
                  <button key={u} onClick={()=>setUnit(u)}
                    style={{padding:"8px 16px",background:unit===u?goldGrad:"transparent",color:unit===u?G.text:G.muted,border:"none",fontWeight:800,fontSize:11,cursor:"pointer"}}>
                    {u.toUpperCase()}
                  </button>
                ))}
              </div>
            ...
            <div style={{...cardStyle,padding:16,marginBottom:14}}>
              <div style={{fontFamily:F.display,fontSize:16,color:G.text,letterSpacing:4,marginBottom:14}}>1RM SETTINGS</div>
                  ...
                  {rm>0&&<div style={{marginTop:7,display:"flex",gap:8,flexWrap:"wrap"}}>
                    {[[0.70,"70%"],[0.85,"85%"],[0.88,"88%"],[0.91,"91%"]].map(([p,lbl])=>(
                      <span key={lbl} style={{fontSize:10,color:G.muted,background:"#0f0f0f",border:`1px solid ${G.border}`,borderRadius:3,padding:"3px 8px"}}>
                        <span style={{color:G.text,fontWeight:700}}>{r25(rm*p, unit)}</span> @ {lbl}
                      </span>
                    ))}
                  </div>}
                </div>
            ...
            <div style={{...cardStyle,padding:16,marginBottom:14}}>
              <button onClick={handleSignOut}
                style={{width:"100%",padding:12,background:"transparent",border:`1px solid ${G.border}`,
                  fontFamily:F.display,fontSize:14,letterSpacing:4,color:G.muted,cursor:"pointer"}}>
                SIGN OUT
              </button>
            </div>
            <div style={{...cardStyle,padding:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div style={{fontSize:10,color:G.text,fontWeight:800,letterSpacing:3}}>MEV/MRV FACTORS</div>
                <div style={{fontSize:12,fontWeight:800,color:G.text}}>{volScore>0?"+":""}{volScore}</div>
              </div>
            ...
            <button onClick={async()=>{await ST.signOut();setProfile(null);setCompleted(new Set());setLogs({});setAccLogs({});setRms({});setMevSel({});}}
              style={{width:"100%",marginTop:14,padding:14,background:"transparent",border:`1px solid ${G.border}`,
                fontFamily:F.display,fontSize:14,letterSpacing:4,color:G.muted,cursor:"pointer"}}>
              SIGN OUT
            </button>
          </div>
        ...
        {NAV.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{flex:1,padding:"10px 4px",background:"transparent",border:"none",
              color:tab===t.id?G.text:G.muted,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,
              borderTop:tab===t.id?`2px solid ${G.text}`:"2px solid transparent",marginTop:-2}}>
            <span style={{fontSize:15}}>{t.icon}</span>
            <span style={{fontFamily:F.display,fontSize:9,letterSpacing:2,color:tab===t.id?G.text:G.muted}}>{t.label.toUpperCase()}</span>
          </button>
        ))}
      </nav>
