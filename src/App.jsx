...
function BarLoaderSheet({seedLbs, onClose}) {
  ...
  const BAR_LB = 45;
  const PLATES_LB = [
    {lb:45,label:"45",name:"Red"},{lb:35,label:"35",name:"Blue"},
    {lb:25,label:"25",name:"Yellow"},{lb:10,label:"10",name:"Green"},
    {lb:5,label:"5",name:"White"},{lb:2.5,label:"2.5",name:"Gray"},
  ];
  const calcL = (target) => {
    ...
    return (
      <div style={{display:"flex",alignItems:"center",gap:2,flexShrink:0}}>
        {plates.map(p=>{
          ...
          return <div key={p.key} style={{width:w,height:h,background:G.border,border:`1px solid ${G.gold}30`,borderRadius:3,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <span style={{fontSize:7,color:G.gold,fontFamily:F.display,writingMode:"vertical-rl",transform:"rotate(180deg)",letterSpacing:1}}>{p.label}</span>
          </div>;
        })}
      </div>
    );
            ...
            <div style={{background:"#0a0a0a",border:`1px solid ${G.border}`,overflow:"hidden",marginBottom:16}}>
              ...
              {loading2.map((count,i)=>count>0?(
                <div key={i} style={{display:"flex",alignItems:"center",padding:"10px 14px",gap:10,borderBottom:`1px solid ${G.border2}`}}>
                  <div style={{width:32,height:32,background:G.border,border:`1px solid ${G.gold}40`,borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <span style={{fontFamily:F.display,fontSize:11,color:G.gold,letterSpacing:1}}>{PLATES_LB[i].label}</span>
                  </div>
                  <div style={{flex:1}}>
...
function SessionModal({s,rms,unit,completed,onToggle,onClose,onLog,logs,accLogs,setAccLog,onTimer}) {
  ...
  const [setsDone,setSetsDone]         = useState({});   // {liftName: count}
  ...
  const getLiftWeight = (ex) => {
    if(!ex.pat||!rms[ex.name]) return null;
  ...
  // Current lift in focus
  const curEx = mainEx[liftIdx];
  const curW  = curEx ? getLiftWeight(curEx) : null;
  const curTotalSets = curEx?.pat?.reps[2] || 3;
  const curDone = setsDone[curEx?.name] || 0;
  const allSetsDone = curDone >= curTotalSets;
  const allMainDone = liftIdx >= mainEx.length;

  // Overall session progress
  const totalMainSets = mainEx.reduce((s,ex)=>s+(ex.pat?.reps[2]||3),0);
  const doneMainSets  = Object.values(setsDone).reduce((a,b)=>a+b,0);
  const progress = totalMainSets > 0 ? doneMainSets/totalMainSets : 0;
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(4,4,4,0.97)",zIndex:200,overflowY:"auto",padding:"12px 10px"}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        {!isDone&&<div style={{display:"flex",gap:2,height:3}}>
          {PHASES.map((p,i)=>(
            <div key={p} style={{flex:1,background:phase===p?goldGrad:PHASES.indexOf(phase)>i?G.goldDim:G.border,transition:"background 0.3s"}}/>
          ))}
        </div>}
        ...
        {/* ══ SESSION ══ */}
        {(phase==="session"||(isDone&&phase!=="wins"))&&(
          <div>
            ...
            {readiness&&readiness.adj!==0&&(
              <div style={{padding:"8px 16px",background:G.card2,borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontFamily:F.body,fontSize:10,color:G.muted}}>— {readiness.profile.msg}</span>
              </div>
            )}

            {/* ── Progress bar ── */}
            {mainEx.length>0&&!allMainDone&&(
              <div style={{padding:"10px 16px 6px",borderBottom:`1px solid ${G.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                  <span style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:3}}>LIFT {liftIdx+1} OF {mainEx.length}</span>
                  <span style={{fontFamily:F.display,fontSize:10,color:G.gold,letterSpacing:2}}>{doneMainSets}/{totalMainSets} SETS</span>
                </div>
            ...
            {/* ── FOCUS MODE: one lift at a time ── */}
            {mainEx.length>0&&!allMainDone&&(
              <div style={{padding:"24px 16px"}}>

                {/* Lift card */}
                <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"28px 20px",textAlign:"center",marginBottom:20}}>
                  <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:4,marginBottom:6}}>
                    {curEx.proto} · {curTotalSets} SETS @ {curEx.pat?.tl}{readiness?.adj?` (${readiness.adj>0?"+":""}${readiness.adj}% ADJ)`:""}
                  </div>
                  <div style={{fontFamily:F.display,fontSize:44,color:G.gold,letterSpacing:3,lineHeight:1,marginBottom:12}}>
                    {curEx.name.toUpperCase()}
                  </div>
                  <div style={{fontFamily:F.display,fontSize:72,color:G.text,lineHeight:1,marginBottom:8}}>
                    {curW||"--"}<span style={{fontSize:22,color:G.muted,marginLeft:4}}>{unit}</span>
                  </div>

                  {/* Warm-up weights */}
                  {curW&&(
                    <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:14,flexWrap:"wrap"}}>
                      {[["WU 1",r25(curW*0.70,unit),"70%"],["WU 2",r25(curW*0.85,unit),"85%"]].map(([lbl,w,pct])=>(
                        <div key={lbl} style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"6px 14px",textAlign:"center"}}>
                          <div style={{fontFamily:F.display,fontSize:9,color:G.muted,letterSpacing:3,marginBottom:2}}>{lbl} · {pct}</div>
                          <div style={{fontFamily:F.display,fontSize:18,color:G.muted,letterSpacing:1}}>{w} {unit}</div>
                        </div>
                      ))}
                      <div style={{background:"#0d0b00",border:`1px solid ${G.gold}40`,padding:"6px 14px",textAlign:"center"}}>
                        <div style={{fontFamily:F.display,fontSize:9,color:G.goldDim,letterSpacing:3,marginBottom:2}}>BACK-OFF</div>
                        <div style={{fontFamily:F.display,fontSize:18,color:G.goldDim,letterSpacing:1}}>{r25(curW*0.88,unit)} {unit}</div>
                      </div>
                    </div>
                  )}

                  {curW&&(
                    <button onClick={()=>setBarTarget(Math.round(curW*2.20462))}
                      style={{background:G.border,border:`1px solid ${G.gold}40`,color:G.gold,padding:"7px 20px",
                        fontFamily:F.display,fontSize:11,letterSpacing:3,cursor:"pointer"}}>
                      🏋 VIEW PLATES
                    </button>
                  )}
                </div>
                ...
                <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:20}}>
                  {Array.from({length:curTotalSets}).map((_,i)=>(
                    <div key={i} style={{width:52,height:52,display:"flex",alignItems:"center",justifyContent:"center",
                      fontFamily:F.display,fontSize:20,
                      background:i<curDone?goldGrad:G.card2,
                      border:`1px solid ${i<curDone?"transparent":G.border}`,
                      color:i<curDone?G.black:G.muted,transition:"all 0.2s"}}>
                      {i<curDone?"✓":i+1}
                    </div>
                  ))}
                </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,marginBottom:10}}>
                      {QUALITY.map(q=>(
                        <button key={q.v} onClick={()=>{
                          setQualityRatings(p=>({...p,[curEx.name]:[...(p[curEx.name]||[]),q.v]}));
                          ...
                          <span style={{fontFamily:F.body,fontSize:7,fontWeight:700,color:qColor(q.v),textTransform:"uppercase",letterSpacing:1,lineHeight:1,textAlign:"center"}}>{q.label}</span>
                        </button>
                      ))}
                    </div>
                    <button onClick={()=>setPendingQuality(null)}
                ...
                /* SET DONE button */
                  ...
                  <button onClick={()=>{
                    const next = (setsDone[curEx.name]||0) + 1;
                    setSetsDone(p=>({...p,[curEx.name]:next}));
                    setPendingQuality(curEx.name);
                ...
                /* NEXT LIFT button */
                ):liftIdx<mainEx.length-1?(
                  <button onClick={()=>{setLiftIdx(i=>i+1);setPendingQuality(null);}}
                    style={{width:"100%",padding:16,background:G.card2,border:`1px solid ${G.gold}`,
                      fontFamily:F.display,fontSize:22,letterSpacing:4,color:G.gold,cursor:"pointer"}}>
                    NEXT LIFT →
                  </button>

                /* ALL MAIN LIFTS DONE → show accessories */
                ):(
                  <button onClick={()=>setLiftIdx(mainEx.length)}
                    style={{width:"100%",padding:16,background:G.card2,border:`1px solid ${G.gold}`,
                      fontFamily:F.display,fontSize:22,letterSpacing:4,color:G.gold,cursor:"pointer"}}>
                    ACCESSORY WORK →
                  </button>
            ...
            {(allMainDone||mainEx.length===0)&&(
              <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:14}}>

                {/* Summary of main lifts done */}
                {mainEx.length>0&&(
                  <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"12px 16px"}}>
                    <div style={{fontFamily:F.display,fontSize:10,color:G.muted,letterSpacing:4,marginBottom:10}}>MAIN LIFTS — DONE</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                      {mainEx.map(ex=>{
                        const w=getLiftWeight(ex);
                        const ratings=qualityRatings[ex.name]||[];
                        const avg=ratings.length?ratings.reduce((a,b)=>a+b,0)/ratings.length:null;
                        return (
                          <div key={ex.name} style={{flex:"1 1 auto",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,borderBottom:`1px solid ${G.border}`,paddingBottom:8}}>
                            <span style={{fontFamily:F.display,fontSize:14,color:G.gold,letterSpacing:2}}>{ex.name}</span>
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              {w&&<span style={{fontFamily:F.display,fontSize:13,color:G.muted,letterSpacing:1}}>{w} {unit}</span>}
                              {avg!==null&&<span style={{fontFamily:F.display,fontSize:16,color:qColor(avg),letterSpacing:1}}>{avg.toFixed(1)}</span>}
                ...
                {accEx.length>0&&(
                  <div>
                    <SH color={G.gold}>ACCESSORY WORK</SH>
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <button onClick={isDone?onClose:handleComplete}
                  style={{width:"100%",padding:16,background:isDone?G.card2:goldGrad,
                    border:isDone?`1px solid ${G.border}`:"none",color:isDone?G.gold:G.black,
                    fontFamily:F.display,fontSize:20,letterSpacing:3,cursor:"pointer"}}>
                  {isDone?"✓ SESSION COMPLETE — CLOSE":"MARK AS COMPLETE"}
                </button>
              </div>
            )}
          </div>
...
const BAR_KG = 25;
const PLATES_DATA = [
  { kg:25,   label:"25",   name:"Red"    },
  { kg:20,   label:"20",   name:"Blue"   },
  { kg:15,   label:"15",   name:"Yellow" },
  { kg:10,   label:"10",   name:"Green"  },
  { kg:5,    label:"5",    name:"White"  },
  { kg:2.5,  label:"2.5",  name:"Black"  },
  { kg:1.25, label:"1.25", name:"Silver" },
];
        ...
        <div style={cs}>
          ...
          {PLATES_DATA.map((p,i)=>loading2[i]>0&&(
            <div key={p.name} style={{display:"flex",alignItems:"center",padding:"12px 16px",borderBottom:`1px solid ${G.border2}`,gap:12}}>
              <div style={{width:36,height:36,background:G.border,border:`1px solid ${G.gold}40`,borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                <span style={{fontFamily:F.display,fontSize:11,color:G.gold,letterSpacing:1}}>{p.label}</span>
              </div>
              <div style={{flex:1}}>
