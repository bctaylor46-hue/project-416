      ...
      {f.opts.map(o => { const sel = mev[f.key]===o.v; return (
        <button key={o.l} onClick={()=>tog(f.key,o.v)}
          style={{padding:"5px 11px",borderRadius:4,border:sel?`1px solid ${G.teal}`:`1px solid ${G.border}`,
            background:sel?"#1a0a28":"#0a0a0a",color:sel?G.teal:G.muted,fontSize:11,cursor:"pointer",fontWeight:sel?700:400}}>
        ...
        <div style={card}>
          ...
          {step===2 && (
            <div>
              ...
              <div style={{background:"#0a0a14",border:`1px solid ${G.border}`,borderRadius:8,padding:"12px 14px",marginBottom:16}}>
...
function MainBlock({ex,rms,unit,sid,onLog,logs,onTimer,readinessAdj=0}) {
  ...
  return (
    <div style={{border:`1px solid ${G.border}`,borderRadius:8,overflow:"hidden"}}>
      ...
      {ROWS.map(row=>(
        <div key={row.ro} style={{display:"flex",alignItems:"center",borderTop:`1px solid ${G.border}`,padding:"9px 14px",background:row.isTop?"#181020":"transparent"}}>
...
function BarLoaderSheet({seedLbs, onClose}) {
  ...
  const PlateRow = ({counts,side}) => {
    if(!counts) return null;
    // Build array heaviest-first (natural PLATES_LB order)
    let plates = [];
    PLATES_LB.forEach((p,i)=>{ for(let j=0;j<counts[i];j++) plates.push({...p,key:`${i}-${j}`}); });
    // Left side: sleeve(left)→collar(right) — reverse so heaviest nearest collar on right
    if(side==="left") plates = [...plates].reverse();
    // Right side: collar(left)→sleeve(right) — natural order so heaviest nearest collar on left
    return (
          ...
          {loading2&&(
            <div style={{background:"#0a0a0a",border:`1px solid ${G.border}`,padding:"16px 12px",marginBottom:12,overflowX:"auto"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",minWidth:"max-content",margin:"0 auto"}}>
                ...
                <PlateRow counts={loading2} side="left"/>
                ...
                <div style={{width:8,height:60,background:G.border,borderRadius:2,flexShrink:0,margin:"0 2px"}}/>
                <PlateRow counts={loading2} side="right"/>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(4,4,4,0.97)",zIndex:200,overflowY:"auto",padding:"12px 10px"}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        {isPR&&phase==="session"&&<div style={{background:"#0d000d",padding:"9px 16px",borderBottom:`1px solid ${G.purple}40`}}>
      ...
      {minimized&&(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
          ...
          <button onClick={()=>done?reset():setRunning(r=>!r)}
            style={{background:done?goldGrad:running?"#3a0808":"#1a1000",border:"none",borderRadius:6,color:done?G.text:G.teal,padding:"6px 10px",fontSize:14,cursor:"pointer"}}>
            {done?"↺":running?"⏸":"▶"}
          </button>
        </div>
      )}
      {!minimized&&(
        <div style={{padding:"12px 14px"}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>done?reset():setRunning(r=>!r)}
              style={{flex:3,padding:"9px",background:done?goldGrad:running?"#2a0808":"#1a1000",
                ...
                <div key={s.id} style={{padding:"10px 0",borderBottom:"1px solid #1e1e1e"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:done&&sl?6:0}}>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:10,background:done?"#1a1000":"#1a1a1a",color:done?G.teal:"#2a2a2a",padding:"2px 8px",borderRadius:3,fontWeight:800}}>S{s.id}</span>
          ...
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {weekData.sessions.map(s=>{
                ...
                return(
                  <div key={s.id} style={{...cardStyle,padding:"13px 14px",border:isDone?`1px solid ${G.teal}30`:undefined}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
                        ...
                        <button onClick={()=>toggleDone(s.id)}
                          style={{background:isDone?"#0a0900":"#0a0a0a",border:isDone?`1px solid ${G.teal}40`:`1px solid ${G.border}`,
