...
function ReadinessGate({onComplete}) {
  ...
  return (
    <div style={{padding:"20px 16px"}}>
      ...
      <p style={{fontFamily:F.body,fontSize:12,color:G.muted,marginBottom:24,lineHeight:1.7}}>No wrong answers. This adapts your session weights — not your character.</p>

      {READINESS_Qs.map(q=>{
        const val=vals[q.id];
        const pct=((val-1)/9)*100;
        const c = q.inverted
          ? (val<=4?G.gold:val<=7?G.goldDim:G.muted)
        ...
        return (
          <div key={q.id} style={{marginBottom:20}}>
            ...
            <input type="range" min={1} max={10} value={val}
              ...
              style={{width:"100%",appearance:"none",WebkitAppearance:"none",height:3,borderRadius:2,
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <div style={{flex:1,height:2,background:G.border}}/>
...
function WinsScreen({readiness,sessionId,completed,onClose}) {
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
        {[
          ...
          <div key={label} style={{background:G.card,border:`1px solid ${G.border}`,padding:"16px 10px"}}>
            ...
            <div style={{fontFamily:F.display,fontSize:26,color:G.gold,letterSpacing:2,lineHeight:1}}>{val}</div>
...
function SessionModal({s,rms,unit,completed,onToggle,onClose,onLog,logs,accLogs,setAccLog,onTimer}) {
  const isDone=completed.has(s.id),fc=G.gold,isPR=s.id>=15;
  ...
  const handleCheckin = (data) => { setReadiness(data); setPhase("session"); };
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        <div style={{background:G.card2,padding:"14px 16px",borderBottom:`1px solid ${G.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <span style={{fontFamily:F.display,fontSize:12,color:fc,letterSpacing:2,background:`${fc}15`,padding:"1px 10px",border:`1px solid ${fc}30`}}>{s.focus.toUpperCase()}</span>
        ...
        {/* Phase progress */}
        {!isDone&&<div style={{display:"flex",gap:3,padding:"0",height:3}}>
          {PHASES.map((p,i)=>(
        ...
        {/* CHECK-IN PHASE */}
        {phase==="checkin"&&!isDone&&(
          <ReadinessGate onComplete={handleCheckin}/>
        )}
        ...
        {(phase==="session"||(isDone&&phase!=="wins"))&&(
          <div>
            ...
            <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:14}}>
              {mainEx.length>0&&(
                <div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    {mainEx.map((ex,i)=><MainBlock key={i} ex={ex} rms={rms} unit={unit} sid={s.id} onLog={onLog} logs={logs} onTimer={onTimer} readinessAdj={readiness?.adj||0}/>)}
                  </div>
                </div>
              )}
              {accEx.length>0&&(
            ...
            <div style={{borderTop:`1px solid ${G.border}`}}>
              <button onClick={()=>setShowBar(b=>!b)}
                style={{width:"100%",padding:"12px 16px",background:"transparent",border:"none",borderBottom:`1px solid ${G.border}`,
                  color:showBar?G.gold:G.muted,fontFamily:F.display,fontSize:14,letterSpacing:3,cursor:"pointer",
                  display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span>🏋 BAR LOADER</span>
                <span style={{fontSize:12}}>{showBar?"▲":"▼"}</span>
              </button>
              {showBar&&<div style={{padding:"14px 16px",borderBottom:`1px solid ${G.border}`}}><BarLoader/></div>}
            </div>
        ...
        {phase==="wins"&&readiness&&(
          <WinsScreen readiness={readiness} sessionId={s.id} completed={completed} onClose={onClose}/>
        )}
