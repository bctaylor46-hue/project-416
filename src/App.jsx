    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
      {f.opts.map(o => { const sel = mev[f.key]===o.v; return (
        <button key={o.l} onClick={()=>tog(f.key,o.v)}
          ...
          {o.l} <span style={{fontSize:9,color:sel?G.goldDim||G.teal:"#333"}}>({o.v>0?"+":""}{o.v})</span>
        </button>
      );})}
    </div>
  </div>
          ...
          return(
            <div key={pat.id} style={{background:G.card,borderRadius:10,border:`1px solid ${G.border}`,padding:14}}>
              ...
              <div style={{background:"#1a1a1a",borderRadius:99,height:6,marginBottom:7,position:"relative",overflow:"visible"}}>
                ...
                {adjMev>0&&<div style={{position:"absolute",top:-3,left:`${Math.min(99,(adjMev/adjMrv)*100)}%`,width:1,height:12,background:G.goldDim||G.teal,opacity:0.6}}/>}
              </div>
...
function MainBlock({ex,rms,unit,sid,onLog,logs,onTimer,readinessAdj=0}) {
  ...
  return (
    <div style={{border:`1px solid ${G.border}`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:"#0d0d0d",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      ...
      {ROWS.map(row=>(
        <div key={row.ro} style={{display:"flex",alignItems:"center",borderTop:`1px solid ${G.border}`,padding:"9px 14px",background:row.isTop?`${G.purple}15`:"transparent"}}>
...
function RpeBlock({ex,sid,accLogs,setAccLog,unit,onTimer,nested=false}) {
  ...
  return (
    <div style={{border:nested?"none":`1px solid #1a3050`,borderRadius:nested?0:8,overflow:"hidden"}}>
      <div style={{background:nested?"#0d0d0d":"#0d0d0d",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:F.display,fontSize:20,color:G.teal,letterSpacing:2,lineHeight:1}}>{ex.name}</span>
          ...
          <div key={i} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:`1px solid #0a1a2a`}}>
            ...
            <div style={{width:82,textAlign:"center",flexShrink:0}}>
              {set.rpe!=null
                ...
                :<span style={{fontSize:10,color:G.goldDim||G.teal,fontStyle:"italic",whiteSpace:"nowrap"}}>{set.note||"= prev"}</span>
              }
            </div>
...
function SupersetBlock({ex,sid,accLogs,setAccLog,unit,onTimer}) {
  return (
    <div style={{border:`1px solid #2a2a2a40`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:"#0d0d0d",padding:"7px 14px",borderBottom:"1px solid #1e1040",display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"4px",background:"#0c0c0c",borderTop:"1px solid #1e1040",borderBottom:"1px solid #1e1040"}}>
        <span style={{fontSize:9,color:"#555555",fontWeight:700,letterSpacing:2}}>⟷ SUPERSET ⟷</span>
      </div>
                ...
                <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${G.border}`}}>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    ...
                    <div style={{background:`${G.teal}15`,border:`1px solid ${G.teal}40`,padding:"4px 8px",textAlign:"center",alignSelf:"center"}}>
                      <div style={{fontFamily:F.display,fontSize:8,color:G.goldDim||G.teal,letterSpacing:1}}>TARGET</div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(4,4,4,0.97)",zIndex:200,overflowY:"auto",padding:"12px 10px"}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
        ...
        {!isDone&&<div style={{display:"flex",gap:2,height:3}}>
          {PHASES.map((p,i)=>(
            <div key={p} style={{flex:1,background:phase===p?goldGrad:PHASES.indexOf(phase)>i?G.goldDim||G.teal:G.border}}/>
          ))}
        </div>}
            ...
            {cur.type==="main"&&(
              <div style={{padding:"24px 16px"}}>
                ...
                <div style={{marginBottom:20}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",background:"#0a0a0a",border:`1px solid ${cur.row.isTop?G.teal:G.border}`}}>
...
function EMOMTimer({proto,exName,onClose}) {
  ...
  return (
    <div style={{position:"fixed",bottom:80,right:12,zIndex:400,background:flash?`rgba(42,122,120,0.15)`:G.card,
      border:`1px solid ${flash?G.teal:G.border}`,borderRadius:12,minWidth:200,maxWidth:260,
...
function CoachDashboard({profile, onSignOut}) {
  ...
  if (selected) {
    ...
    return (
      <div style={{minHeight:"100vh",background:"#080808",color:"#F0F0F0",fontFamily:"system-ui,sans-serif",padding:16}}>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080808",color:"#F0F0F0",fontFamily:"system-ui,sans-serif"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              {PROGRAM.map(w=>{const wd=w.sessions.filter(s=>completed.has(s.id)).length,act=activeWeek===w.week;return(
                <button key={w.week} onClick={()=>setActiveWeek(w.week)}
                  ...
                  <div style={{fontSize:10,color:act?G.goldDim||G.teal:"#2a2a2a",marginTop:2}}>{wd}/4</div>
                </button>
              );})}
            </div>
                        ...
                        {accEx.length>0&&<div>
                          ...
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {accEx.map((e,i)=>e.type==="superset"
                              ?<span key={i} style={{fontSize:11,color:G.goldDim||G.teal,background:"#0d0d0d",borderRadius:3,padding:"2px 8px"}}>{e.exA.name}+{e.exB.name}</span>
        ...
        {NAV.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{flex:1,padding:"10px 4px",background:"transparent",border:"none",
              ...
              borderTop:tab===t.id?`2px solid ${G.teal}`:"2px solid transparent",marginTop:-2}}>
