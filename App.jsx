...
function SessionModal({s,rms,unit,completed,onToggle,onClose,onLog,logs,accLogs,setAccLog,onTimer}) {
  ...
  // Per-lift weight for focus mode
      ...
      <div style={{maxWidth:580,margin:"0 auto",background:G.card,border:`1px solid ${G.border}`,overflow:"hidden"}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            {phase==="session"&&(
              <button onClick={()=>setFocusMode(f=>!f)}
                style={{background:focusMode?`${G.gold}20`:"transparent",border:`1px solid ${focusMode?G.gold:G.border}`,
                  color:focusMode?G.gold:G.muted,padding:"4px 10px",fontFamily:F.display,fontSize:10,letterSpacing:2,cursor:"pointer"}}>
                {focusMode?"⚡ FOCUS":"📋 ALL"}
              </button>
            )}
        ...
        {(phase==="session"||(isDone&&phase!=="wins"))&&(
          <div>
            ...
            {/* FOCUS MODE — one main lift at a time */}
            {focusMode&&mainEx.length>0&&(()=>{
              const focusIdx = mainEx.findIndex(ex => {
                const key=ex.name;
                const done=(qualityRatings[key]||[]).length;
                return done < ex.pat?.reps[2] || !qualityRatings[key];
              });
              const activeIdx = focusIdx===-1?mainEx.length-1:focusIdx;
              const ex = mainEx[activeIdx];
              const w = getLiftWeight(ex);
              const ratings = qualityRatings[ex.name]||[];
              const totalSets = ex.pat?.reps[2]||3;
              const allDone = ratings.length>=totalSets;

              return (
                <div style={{padding:"20px 16px"}}>
                  <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:3,marginBottom:4}}>
                    LIFT {activeIdx+1} OF {mainEx.length}
                  </div>
                  <div style={{background:G.card2,border:`1px solid ${G.border}`,padding:"24px 20px",textAlign:"center",marginBottom:16}}>
                    <div style={{fontFamily:F.display,fontSize:40,color:G.gold,letterSpacing:3,lineHeight:1,marginBottom:8}}>{ex.name.toUpperCase()}</div>
                    <div style={{fontFamily:F.display,fontSize:64,color:G.text,lineHeight:1,marginBottom:6}}>
                      {w||"--"}<span style={{fontSize:20,color:G.muted}}>{unit}</span>
                    </div>
                    <div style={{fontFamily:F.body,fontSize:13,color:G.muted,marginBottom:w?10:0}}>
                      {totalSets} sets @ {ex.pat?.tl}{readiness?.adj?` (${readiness.adj}% adj)`:""} · {ex.proto}
                    </div>
                    {w&&(
                      <button onClick={()=>setBarTarget(Math.round(w*2.20462))}
                        style={{background:G.border,border:`1px solid ${G.gold}40`,color:G.gold,padding:"6px 16px",
                          fontFamily:F.display,fontSize:11,letterSpacing:3,cursor:"pointer",marginTop:4}}>
                        🏋 VIEW PLATES
                      </button>
                    )}
                  </div>
                  ...
                  <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:16}}>
                    {Array.from({length:totalSets}).map((_,i)=>(
                      <div key={i} style={{width:44,height:44,display:"flex",alignItems:"center",justifyContent:"center",
                        ...
                        background:i<ratings.length?goldGrad:G.card2,
                        border:`1px solid ${i<ratings.length?"transparent":G.border}`,
                        color:i<ratings.length?G.black:G.muted,transition:"all 0.2s"}}>
                        {i<ratings.length?"✓":i+1}
                      </div>
                    ))}
                  </div>

                  {/* Quality rating or next set button */}
                  {pendingQuality===ex.name?(
                    <div>
                      <div style={{fontFamily:F.display,fontSize:11,color:G.muted,letterSpacing:4,textAlign:"center",marginBottom:10}}>HOW DID THAT SET FEEL?</div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5,marginBottom:8}}>
                        {QUALITY.map(q=>(
                          <button key={q.v} onClick={()=>{
                            setQualityRatings(p=>({...p,[ex.name]:[...(p[ex.name]||[]),q.v]}));
                            setPendingQuality(null);
                          }} style={{padding:"10px 0",background:`${qColor(q.v)}22`,border:`1px solid ${qColor(q.v)}66`,cursor:"pointer",
                            display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                            <span style={{fontFamily:F.display,fontSize:20,color:qColor(q.v)}}>{q.v}</span>
                            <span style={{fontFamily:F.body,fontSize:7,fontWeight:700,color:qColor(q.v),textTransform:"uppercase",letterSpacing:1,textAlign:"center",lineHeight:1}}>{q.label}</span>
                          </button>
                        ))}
                      </div>
                      <button onClick={()=>setPendingQuality(null)}
                        style={{width:"100%",padding:"8px",background:"transparent",border:`1px solid ${G.border}`,
                          fontFamily:F.body,fontSize:11,color:G.muted,cursor:"pointer"}}>skip</button>
                    </div>
                  ):!allDone?(
                    <button onClick={()=>{
                      setQualityRatings(p=>({...p,[ex.name]:[...(p[ex.name]||[])]}));
                      setPendingQuality(ex.name);
                    }} style={{width:"100%",padding:14,background:goldGrad,border:"none",
                      fontFamily:F.display,fontSize:20,letterSpacing:4,color:G.black,cursor:"pointer"}}>
                      ✓ SET {ratings.length+1} DONE
                    </button>
                  ):activeIdx<mainEx.length-1?(
                    <button onClick={()=>{
                      const nextEx=mainEx[activeIdx+1];
                      setQualityRatings(p=>({...p,[nextEx.name]:[]}));
                      setPendingQuality(null);
                    }} style={{width:"100%",padding:14,background:G.card2,border:`1px solid ${G.gold}`,
                      fontFamily:F.display,fontSize:20,letterSpacing:4,color:G.gold,cursor:"pointer"}}>
                      NEXT LIFT →
                    </button>
                  ):(
                  ...
                  <div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {mainEx.map((ex,i)=>{
                        const w=getLiftWeight(ex);
                        return (
                          <div key={i} style={{border:`1px solid ${G.border}`,cursor:"pointer"}}
                            onClick={()=>{setFocusMode(true);}}>
                            <div style={{background:G.card2,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                              <span style={{fontFamily:F.display,fontSize:18,color:G.gold,letterSpacing:2}}>{ex.name}</span>
                              <div style={{display:"flex",alignItems:"center",gap:8}}>
                                {w&&<span style={{fontFamily:F.display,fontSize:16,color:G.text,letterSpacing:1}}>{w} {unit}</span>}
                                {w&&<button onClick={e=>{e.stopPropagation();setBarTarget(Math.round(w*2.20462));}}
                                  style={{background:G.border,border:`1px solid ${G.gold}40`,color:G.gold,padding:"3px 8px",fontFamily:F.display,fontSize:10,letterSpacing:2,cursor:"pointer"}}>
                                  🏋
                                </button>}
                                <span style={{fontFamily:F.display,fontSize:12,color:G.gold,background:`${G.gold}15`,padding:"1px 8px",letterSpacing:2}}>{ex.proto}</span>
                              </div>
                            </div>
                            <MainBlock ex={ex} rms={rms} unit={unit} sid={s.id} onLog={onLog} logs={logs} onTimer={onTimer} readinessAdj={readiness?.adj||0}/>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div style={{padding:"12px 16px",borderTop:`1px solid ${G.border}`}}>
              <button onClick={isDone?onClose:handleComplete}
                style={{width:"100%",padding:14,background:isDone?G.card2:goldGrad,
                  border:isDone?`1px solid ${G.border}`:"none",color:isDone?G.gold:G.black,
                  fontFamily:F.display,fontSize:18,letterSpacing:3,cursor:"pointer"}}>
